...
function updateenv() {
    ...
    if [ $? -ne 0 ]; then
        echo "Failed installing Coingro"
        exit 1
    fi
...
function config() {
    echo_block "Please use 'coingro new-config -c config.json' to generate a new configuration file."
}
...
function install() {
    ...
    echo "You can now use the bot by executing 'source .env/bin/activate; coingro <subcommand>'."
    echo "You can see the list of available bot sub-commands by executing 'source .env/bin/activate; coingro --help'."
    echo "You verify that coingro is installed successfully by running 'source .env/bin/activate; coingro --version'."
}
...
function help() {
    echo "usage:"
    echo "	-i,--install    Install coingro from scratch"
