...
from coingro.exchange.common import (
    API_FETCH_ORDER_RETRY_COUNT,
    API_RETRY_COUNT,
    calculate_backoff,
    remove_credentials,
)
from coingro.exchange.exchange import (
    date_minus_candles,
    market_is_active,
...
def ccxt_exceptionhandlers(
    ...
    **kwargs,
    ...
    with patch("coingro.exchange.common.time.sleep"):
...
async def async_ccxt_exception(
    ...
    with patch("coingro.exchange.common.asyncio.sleep", get_mock_coro(None)):
        with pytest.raises(DDosProtection):
...
def test_validate_credentials(default_conf, mocker, exchange_name):
    ...
    api_mock.check_required_credentials = MagicMock(
        side_effect=ccxt.AuthenticationError("DeadBeef")
    )
...
def test_init_ccxt_kwargs(default_conf, mocker, caplog):
    mocker.patch("coingro.exchange.Exchange._load_markets", MagicMock(return_value={}))
    mocker.patch("coingro.exchange.Exchange.validate_stakecurrency")
    aei_mock = mocker.patch("coingro.exchange.Exchange.additional_exchange_init")
...
def test_exchange_resolver(default_conf, mocker, caplog):
    mocker.patch("coingro.exchange.Exchange._init_ccxt", MagicMock(return_value=MagicMock()))
    mocker.patch("coingro.exchange.Exchange._load_async_markets")
...
def test_validate_order_time_in_force(default_conf, mocker, caplog):
    ...
    # Patch to see if this will pass if the values are in the cg dict
    ex._cg_has.update({"order_time_in_force": ["gtc", "fok", "ioc"]})
    ex.validate_order_time_in_force(tif2)


@pytest.mark.parametrize(
    "amount,precision_mode,precision,contract_size,expected,trading_mode",
    [
...
def test_amount_to_precision(
    default_conf, mocker, amount, precision_mode, precision, contract_size, expected, trading_mode
):
    """
    Test rounds down
    """
    ...
    markets = PropertyMock(
        return_value={
            "ETH/BTC": {"contractSize": contract_size, "precision": {"amount": precision}}
        }
    )
    ...
    # TICK_SIZE = 4
    mocker.patch(
        "coingro.exchange.Exchange.precisionMode", PropertyMock(return_value=precision_mode)
    )
    mocker.patch("coingro.exchange.Exchange.markets", markets)
...
def test_set_sandbox(default_conf, mocker):
    ...
    api_mock.load_markets = MagicMock(
        return_value={"ETH/BTC": "", "LTC/BTC": "", "XRP/BTC": "", "NEO/BTC": ""}
    )
    url_mock = PropertyMock(
        return_value={"test": "api-public.sandbox.gdax.com", "api": "https://api.gdax.com"}
    )
...
def test__load_async_markets(default_conf, mocker, caplog):
    ...
    mocker.patch("coingro.exchange.Exchange.validate_pairs")
    mocker.patch("coingro.exchange.Exchange.validate_timeframes")
    mocker.patch("coingro.exchange.Exchange._load_markets")
    mocker.patch("coingro.exchange.Exchange.validate_stakecurrency")
    mocker.patch("coingro.exchange.Exchange.validate_pricing")
...
def test__load_markets(default_conf, mocker, caplog):
    ...
    mocker.patch("coingro.exchange.Exchange._init_ccxt", MagicMock(return_value=api_mock))
    mocker.patch("coingro.exchange.Exchange.validate_pairs")
    mocker.patch("coingro.exchange.Exchange.validate_timeframes")
    mocker.patch("coingro.exchange.Exchange._load_async_markets")
    mocker.patch("coingro.exchange.Exchange.validate_stakecurrency")
...
def test_validate_pairs_exception(default_conf, mocker, caplog):
    caplog.set_level(logging.INFO)
    api_mock = MagicMock()
    mocker.patch("coingro.exchange.Exchange.name", PropertyMock(return_value="Binance"))

    type(api_mock).markets = PropertyMock(return_value={})
    mocker.patch("coingro.exchange.Exchange._init_ccxt", api_mock)
    mocker.patch("coingro.exchange.Exchange.validate_timeframes")
    mocker.patch("coingro.exchange.Exchange.validate_stakecurrency")
    mocker.patch("coingro.exchange.Exchange.validate_pricing")
    mocker.patch("coingro.exchange.Exchange._load_async_markets")
    ...
    mocker.patch("coingro.exchange.Exchange.markets", PropertyMock(return_value={}))
    Exchange(default_conf)
...
def test_validate_timeframes_failed(default_conf, mocker):
    ...
    mocker.patch("coingro.exchange.Exchange._init_ccxt", MagicMock(return_value=api_mock))
    mocker.patch("coingro.exchange.Exchange._load_markets", MagicMock(return_value={}))
    mocker.patch("coingro.exchange.Exchange.validate_pairs", MagicMock())
    with pytest.raises(
        OperationalException, match=r"Invalid timeframe '3m'. This exchange supports.*"
    ):
        Exchange(default_conf)
    default_conf["timeframe"] = "15s"

    with pytest.raises(
        OperationalException, match=r"Timeframes < 1m are currently not supported by Coingro."
    ):
        Exchange(default_conf)
...
def test_validate_timeframes_emulated_ohlcvi_2(default_conf, mocker):
    ...
    mocker.patch("coingro.exchange.Exchange._init_ccxt", MagicMock(return_value=api_mock))
    mocker.patch(
        "coingro.exchange.Exchange._load_markets", MagicMock(return_value={"timeframes": None})
    )
    mocker.patch("coingro.exchange.Exchange.validate_pairs", MagicMock())
    mocker.patch("coingro.exchange.Exchange.validate_stakecurrency")
    with pytest.raises(
...
def test_validate_timeframes_not_in_config(default_conf, mocker):
    ...
    mocker.patch("coingro.exchange.Exchange.validate_stakecurrency")
    mocker.patch("coingro.exchange.Exchange.validate_pricing")
    mocker.patch("coingro.exchange.Exchange.validate_required_startup_candles")
    Exchange(default_conf)
...
def test_validate_pricing(default_conf, mocker):
    ...
    mocker.patch("coingro.exchange.Exchange._init_ccxt", MagicMock(return_value=api_mock))
    mocker.patch("coingro.exchange.Exchange._load_markets", MagicMock(return_value={}))
    mocker.patch("coingro.exchange.exchange.Exchange.validate_trading_mode_and_margin_mode")
    mocker.patch("coingro.exchange.Exchange.validate_pairs")
    mocker.patch("coingro.exchange.Exchange.validate_timeframes")
    mocker.patch("coingro.exchange.Exchange.validate_stakecurrency")
    mocker.patch("coingro.exchange.Exchange.name", "Binance")
...
def test_validate_required_startup_candles(default_conf, mocker, caplog):
    ...
    # Emulate kraken mode
    ex._cg_has["ohlcv_has_history"] = False
    with pytest.raises(
        OperationalException,
...
def test_create_dry_run_order_limit_fill(
    ...
    order = exchange.create_dry_run_order(
        pair="LTC/USDT", ordertype="limit", side=side, amount=1, rate=startprice, leverage=1.0
    )
    ...
    mocker.patch(
        "coingro.exchange.Exchange.fetch_l2_order_book", return_value={"asks": [], "bids": []}
    )
...
def test_create_dry_run_order_market_fill(
    ...
    order = exchange.create_dry_run_order(
        pair="LTC/USDT", ordertype="market", side=side, amount=amount, rate=rate, leverage=1.0
    )
    assert "id" in order
    assert f"dry_run_{side}_" in order["id"]
    assert order["side"] == side
...
def test_fetch_positions(default_conf, mocker, exchange_name):
    mocker.patch("coingro.exchange.Exchange.validate_trading_mode_and_margin_mode")
    api_mock = MagicMock()
...
def test_get_historic_ohlcv(default_conf, mocker, caplog, exchange_name, candle_type):
    ...
    ret = exchange.get_historic_ohlcv(
        pair, "5m", int((arrow.utcnow().int_timestamp - since) * 1000), candle_type=candle_type
    )
...
async def test__async_get_candle_history(default_conf, mocker, caplog, exchange_name):
    ...
    with pytest.raises(
        ...
        await exchange._async_get_candle_history(
            pair, "5m", CandleType.SPOT, (arrow.utcnow().int_timestamp - 2000) * 1000
        )
...
async def test__async_kucoin_get_candle_history(default_conf, mocker, caplog):
    from coingro.exchange.common import _reset_logging_mixin
    ...
    mocker.patch("coingro.exchange.Exchange.name", PropertyMock(return_value="KuCoin"))
...
def test_fetch_l2_order_book(default_conf, mocker, order_book_l2, exchange_name):
    ...
    for val in [1, 5, 10, 12, 20, 50, 100]:
        ...
        if not exchange._cg_has["l2_limit_range"] or val in exchange._cg_has["l2_limit_range"]:
            assert api_mock.fetch_l2_order_book.call_args_list[0][0][1] == val
        else:
            next_limit = exchange.get_next_limit_in_list(val, exchange._cg_has["l2_limit_range"])
...
@pytest.mark.parametrize(
    "side,ask,bid,last,last_ab,expected",
    [
...
def test_get_entry_rate(
    mocker, default_conf, caplog, side, ask, bid, last, last_ab, expected
) -> None:
    caplog.set_level(logging.DEBUG)
    if last_ab is None:
...
def test_get_exit_rate(
    default_conf, mocker, caplog, side, bid, ask, last, last_ab, expected
) -> None:
    caplog.set_level(logging.DEBUG)
...
async def test___async_get_candle_history_sort(default_conf, mocker, exchange_name):
    ...
    exchange = get_patched_exchange(mocker, default_conf, id=exchange_name)
    exchange._api_async.fetch_ohlcv = get_mock_coro(ohlcv)
    sort_mock = mocker.patch("coingro.exchange.exchange.sorted", MagicMock(side_effect=sort_data))
    ...
    exchange._api_async.fetch_ohlcv = get_mock_coro(ohlcv)
    # Reset sort mock
    sort_mock = mocker.patch("coingro.exchange.sorted", MagicMock(side_effect=sort_data))
    # Test the OHLCV data sort
    res = await exchange._async_get_candle_history(
        "ETH/BTC", default_conf["timeframe"], CandleType.SPOT
    )
...
async def test__async_get_trade_history_id(
    ...
    assert exchange._cg_has["trades_pagination_arg"] in fetch_trades_cal[1][1]["params"]
...
def test_get_historic_trades_notsupported(
    ...
    mocker.patch("coingro.exchange.Exchange.exchange_has", return_value=False)
    exchange = get_patched_exchange(mocker, default_conf, id=exchange_name)
...
def test_cancel_stoploss_order_with_result(default_conf, mocker, exchange_name):
    default_conf["dry_run"] = False
    mocker.patch("coingro.exchange.Exchange.fetch_stoploss_order", return_value={"for": 123})
    mocker.patch("coingro.exchange.Ftx.fetch_stoploss_order", return_value={"for": 123})
    mocker.patch("coingro.exchange.Gateio.fetch_stoploss_order", return_value={"for": 123})
    exchange = get_patched_exchange(mocker, default_conf, id=exchange_name)

    res = {"fee": {}, "status": "canceled", "amount": 1234}
    mocker.patch("coingro.exchange.Exchange.cancel_stoploss_order", return_value=res)
    mocker.patch("coingro.exchange.Ftx.cancel_stoploss_order", return_value=res)
    mocker.patch("coingro.exchange.Gateio.cancel_stoploss_order", return_value=res)
    co = exchange.cancel_stoploss_order_with_result(order_id="_", pair="TKN/BTC", amount=555)
    assert co == res

    mocker.patch("coingro.exchange.Exchange.cancel_stoploss_order", return_value="canceled")
    mocker.patch("coingro.exchange.Ftx.cancel_stoploss_order", return_value="canceled")
    mocker.patch("coingro.exchange.Gateio.cancel_stoploss_order", return_value="canceled")
    # Fall back to fetch_stoploss_order
    co = exchange.cancel_stoploss_order_with_result(order_id="_", pair="TKN/BTC", amount=555)
    assert co == {"for": 123}

    exc = InvalidOrderException("")
    mocker.patch("coingro.exchange.Exchange.fetch_stoploss_order", side_effect=exc)
    mocker.patch("coingro.exchange.Ftx.fetch_stoploss_order", side_effect=exc)
    mocker.patch("coingro.exchange.Gateio.fetch_stoploss_order", side_effect=exc)
    co = exchange.cancel_stoploss_order_with_result(order_id="_", pair="TKN/BTC", amount=555)
    ...
    with pytest.raises(InvalidOrderException):
        ...
        mocker.patch("coingro.exchange.Exchange.cancel_stoploss_order", side_effect=exc)
        mocker.patch("coingro.exchange.Ftx.cancel_stoploss_order", side_effect=exc)
        mocker.patch("coingro.exchange.Gateio.cancel_stoploss_order", side_effect=exc)
        exchange = get_patched_exchange(mocker, default_conf, id=exchange_name)
...
def test_fetch_order(default_conf, mocker, exchange_name, caplog):
    ...
    exchange = get_patched_exchange(mocker, default_conf, api_mock, id=exchange_name)
    with patch("coingro.exchange.common.time.sleep") as tm:
        with pytest.raises(InvalidOrderException):
            exchange.fetch_order(order_id="_", pair="TKN/BTC")
...
def test_get_trades_for_order(default_conf, mocker, exchange_name, trading_mode, amount):
    ...
    mocker.patch("coingro.exchange.Exchange.exchange_has", MagicMock(return_value=False))
...
def test_merge_cg_has_dict(default_conf, mocker):
    mocker.patch.multiple(
        "coingro.exchange.Exchange",
    ...
    ex = Exchange(default_conf)
    assert ex._cg_has == Exchange._cg_has_default

    ex = Kraken(default_conf)
    assert ex._cg_has != Exchange._cg_has_default
    assert ex._cg_has["trades_pagination"] == "id"
    assert ex._cg_has["trades_pagination_arg"] == "since"

    # Binance defines different values
    ex = Binance(default_conf)
    assert ex._cg_has != Exchange._cg_has_default
    assert ex._cg_has["stoploss_on_exchange"]
    assert ex._cg_has["order_time_in_force"] == ["gtc", "fok", "ioc"]
    assert ex._cg_has["trades_pagination"] == "id"
    assert ex._cg_has["trades_pagination_arg"] == "fromId"

    conf = copy.deepcopy(default_conf)
    conf["exchange"]["_cg_has_params"] = {"DeadBeef": 20, "stoploss_on_exchange": False}
    # Use settings from configuration (overriding stoploss_on_exchange)
    ex = Binance(conf)
    assert ex._cg_has != Exchange._cg_has_default
    assert not ex._cg_has["stoploss_on_exchange"]
    assert ex._cg_has["DeadBeef"] == 20
...
def test_get_markets_error(default_conf, mocker):
    ex = get_patched_exchange(mocker, default_conf)
    mocker.patch("coingro.exchange.Exchange.markets", PropertyMock(return_value=None))
...
def test_ohlcv_candle_limit(default_conf, mocker, exchange_name):
    exchange = get_patched_exchange(mocker, default_conf, id=exchange_name)
    timeframes = ("1m", "5m", "1h")
    expected = exchange._cg_has["ohlcv_candle_limit"]
    for timeframe in timeframes:
        if "ohlcv_candle_limit_per_timeframe" in exchange._cg_has:
            expected = exchange._cg_has["ohlcv_candle_limit_per_timeframe"][timeframe]
...
def test_extract_cost_curr_rate(mocker, default_conf, order, expected) -> None:
    mocker.patch("coingro.exchange.Exchange.calculate_fee_rate", MagicMock(return_value=0.01))
    ex = get_patched_exchange(mocker, default_conf)
...
def test_calculate_fee_rate(mocker, default_conf, order, expected, unknown_fee_rate) -> None:
    mocker.patch("coingro.exchange.Exchange.fetch_ticker", return_value={"last": 0.081})
    if unknown_fee_rate:
...
def test__get_funding_fees_from_exchange(default_conf, mocker, exchange_name):
    ...
    # mocker.patch('coingro.exchange.Exchange.get_funding_fees', lambda pair, since: y)
    exchange = get_patched_exchange(mocker, default_conf, api_mock, id=exchange_name)
...
@pytest.mark.parametrize(
    "exchange_name,trading_mode",
    [("binance", TradingMode.FUTURES), ("ftx", TradingMode.MARGIN), ("ftx", TradingMode.FUTURES)],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "bibox",
            "margin",
            {"has": {"fetchCurrencies": False}, "options": {"defaultType": "margin"}},
        ),
        (
            "bibox",
            "futures",
            {"has": {"fetchCurrencies": False}, "options": {"defaultType": "swap"}},
        ),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # TODO: Uncoment once _calculate_funding_fees can pas time_in_ratio to exchange._get_funding_fee  # noqa: E501
...
def test__fetch_and_calculate_funding_fees(
    ...
    mocker.patch(
        "coingro.exchange.Exchange.timeframes", PropertyMock(return_value=["1h", "4h", "8h"])
    )
...
def test__fetch_and_calculate_funding_fees_datetime_called(
    ...
    type(api_mock).has = PropertyMock(return_value={"fetchOHLCV": True})
    type(api_mock).has = PropertyMock(return_value={"fetchFundingRateHistory": True})
    mocker.patch("coingro.exchange.Exchange.timeframes", PropertyMock(return_value=["4h", "8h"]))
    exchange = get_patched_exchange(mocker, default_conf, api_mock, id=exchange)
...
def test__get_contract_size(mocker, default_conf, pair, expected_size, trading_mode):
    ...
    exchange = get_patched_exchange(mocker, default_conf, api_mock)
    mocker.patch(
        "coingro.exchange.Exchange.markets",
        {
            "LTC/USD": {
                "symbol": "LTC/USD",
                "contractSize": None,
...
def test_get_max_leverage_futures(default_conf, mocker, leverage_tiers):
    ...
    with pytest.raises(InvalidOrderException, match=r"Amount 1000000000.01 too high for BTC/USDT"):
