...
def test_amount_to_precision(
    ...
    # TICK_SIZE = 4
    mocker.patch('coingro.exchange.Exchange.precisionMode',
...
def test_validate_timeframes_failed(default_conf, mocker):
    ...
    with pytest.raises(OperationalException,
                       match=r"Timeframes < 1m are currently not supported by Coingro."):
        Exchange(default_conf)
...
def test_merge_cg_has_dict(default_conf, mocker):
    ...
    conf = copy.deepcopy(default_conf)
    conf['exchange']['_cg_has_params'] = {"DeadBeef": 20,
                                          "stoploss_on_exchange": False}
...
def test__fetch_and_calculate_funding_fees(
    ...
    mocker.patch('coingro.exchange.Exchange.timeframes', PropertyMock(
...
def test__get_contract_size(mocker, default_conf, pair, expected_size, trading_mode):
    ...
    default_conf['margin_mode'] = 'isolated'
    exchange = get_patched_exchange(mocker, default_conf, api_mock)
    mocker.patch('coingro.exchange.Exchange.markets', {
        'LTC/USD': {
            'symbol': 'LTC/USD',
            'contractSize': None,
        },
