...
from coingro.exchange import timeframe_to_minutes, timeframe_to_prev_date
from coingro.exchange.exchange import timeframe_to_msecs
...
def exchange_futures(request, exchange_conf, class_mocker):
    if not EXCHANGES[request.param].get("futures") is True:
        ...
        class_mocker.patch("coingro.exchange.binance.Binance.fill_leverage_tiers")
        class_mocker.patch("coingro.exchange.exchange.Exchange.fetch_trading_fees")
        class_mocker.patch("coingro.exchange.okx.Okx.additional_exchange_init")
        exchange = ExchangeResolver.load_exchange(request.param, exchange_conf, validate=True)
...
class TestCCXTExchange:
    ...
    def test_ccxt_fetch_l2_orderbook(self, exchange):
        ...
        l2_limit_range = exchange._cg_has["l2_limit_range"]
        l2_limit_range_required = exchange._cg_has["l2_limit_range_required"]
    ...
    def test_ccxt_fetch_ohlcv(self, exchange):
        ...
        # Assume 90% uptime ...
        assert (
            len(exchange.klines(pair_tf))
            > exchange.ohlcv_candle_limit(timeframe, CandleType.SPOT) * 0.90
        )
    ...
    def test_ccxt__async_get_candle_history(self, exchange):
        ...
        if not exchange._cg_has["ohlcv_has_history"] or exchangename == "bittrex":
            return
        pair = EXCHANGES[exchangename]["pair"]
        timeframe = EXCHANGES[exchangename]["timeframe"]
    ...
    def test_ccxt_fetch_funding_rate_history(self, exchange_futures):
        ...
        timeframe_ff = exchange._cg_has.get(
            "funding_fee_timeframe", exchange._cg_has["mark_ohlcv_timeframe"]
        )
