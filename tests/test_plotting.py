...
def test_generate_candlestick_graph_no_signals_no_trades(default_conf, mocker, testdatadir, caplog):
    row_mock = mocker.patch(
        "coingro.plot.plotting.add_indicators", MagicMock(side_effect=fig_generating_mock)
    )
    trades_mock = mocker.patch(
        "coingro.plot.plotting.plot_trades", MagicMock(side_effect=fig_generating_mock)
    )
...
def test_generate_Plot_filename():
    ...
    assert fn == "coingro-plot-UNITTEST_BTC-5m.html"


def test_generate_plot_file(mocker, caplog):
    fig = generate_empty_figure()
    plot_mock = mocker.patch("coingro.plot.plotting.plot", MagicMock())
    store_plot_file(
        fig, filename="coingro-plot-UNITTEST_BTC-5m.html", directory=Path("user_data/plot")
    )

    expected_fn = str(Path("user_data/plot/coingro-plot-UNITTEST_BTC-5m.html"))
...
def test_generate_profit_graph(testdatadir):
    ...
    assert fig.layout.title.text == "Coingro Profit plot"
...
def test_start_plot_dataframe(mocker):
    aup = mocker.patch("coingro.plot.plotting.load_and_plot_trades", MagicMock())
    args = [
        "plot-dataframe",
        "--config",
...
def test_load_and_plot_trades(default_conf, mocker, caplog, testdatadir):
    ...
    mocker.patch.multiple(
        "coingro.plot.plotting", generate_candlestick_graph=candle_mock, store_plot_file=store_mock
    )
...
def test_start_plot_profit(mocker):
    aup = mocker.patch("coingro.plot.plotting.plot_profit", MagicMock())
    args = [
        "plot-profit",
        "--config",
        "config_examples/config_bittrex.example.json",
...
def test_plot_profit(default_conf, mocker, testdatadir):
    ...
    store_mock = MagicMock()
    mocker.patch.multiple(
        "coingro.plot.plotting", generate_profit_graph=profit_mock, store_plot_file=store_mock
    )
    with pytest.raises(
