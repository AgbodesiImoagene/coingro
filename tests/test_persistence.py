...
def test_init_custom_db_url(default_conf, tmpdir):
    ...
    filename = f"{tmpdir}/coingro2_test.sqlite"
...
def test_init_dryrun_db(default_conf, tmpdir):
    filename = f"{tmpdir}/coingro2_prod.sqlite"
    assert not Path(filename).is_file()
...
@pytest.mark.usefixtures("init_persistence")
def test_borrowed(
    limit_buy_order_usdt, limit_sell_order_usdt, fee, caplog, is_short, lev, borrowed, trading_mode
):
    """
...
def test_update_order_from_ccxt(caplog):
    ...
    assert o.cg_pair == "ADA/USDT"
    assert o.cg_order_side == "buy"
...
def test_select_order(fee, is_short):
    ...
    trades[4].orders[1].cg_order_side = trades[4].exit_side
    order = trades[4].select_order(trades[4].exit_side, True)
    assert order is not None

    trades[4].orders[1].cg_order_side = "stoploss"
    order = trades[4].select_order("stoploss", None)
    assert order is not None
    assert order.cg_order_side == "stoploss"
...
def test_select_filled_orders(fee):
    ...
    assert order.cg_order_side == "buy"
