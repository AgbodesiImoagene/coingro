...
from coingro.data.converter import (
...
def test_ohlcv_fill_up_missing_data(testdatadir, caplog):
    ...
    assert log_has_re(
        f"Missing data fillup for UNITTEST/BTC: before: " f"{len(data)} - after: {len(data2)}.*",
        caplog,
    )
...
def test_ohlcv_fill_up_missing_data2(caplog):
    ...
    ticks = [
        ...
        [1511687100000, 8.891e-05, 8.893e-05, 8.875e-05, 8.877e-05, 2251],  # 9:05:00
        [1511687400000, 8.877e-05, 8.883e-05, 8.895e-05, 8.817e-05, 123551],  # 9:10:00
    ]
...
def test_ohlcv_drop_incomplete(caplog):
    ...
    ticks = [
        ...
        [1559923200000, 8.891e-05, 8.893e-05, 8.875e-05, 8.877e-05, 2251],  # 2019-06-06
        [1560009600000, 8.877e-05, 8.883e-05, 8.895e-05, 8.817e-05, 123551],  # 2019-06-07
    ]
    caplog.set_level(logging.DEBUG)
...
def test_convert_trades_format(default_conf, testdatadir, tmpdir):
    tmpdir1 = Path(tmpdir)
    files = [
        {"old": tmpdir1 / "XRP_ETH-trades.json.gz", "new": tmpdir1 / "XRP_ETH-trades.json"},
        {"old": tmpdir1 / "XRP_OLD-trades.json.gz", "new": tmpdir1 / "XRP_OLD-trades.json"},
    ]
    for file in files:
...
def test_convert_ohlcv_format(default_conf, testdatadir, tmpdir, file_base, candletype):
    ...
    convert_ohlcv_format(
        default_conf, convert_from="json", convert_to="jsongz", erase=False, candle_type=candletype
    )
    ...
    # Convert back
    convert_ohlcv_format(
        default_conf, convert_from="jsongz", convert_to="json", erase=True, candle_type=candletype
    )
    for file in files_temp:
        assert file.exists()
