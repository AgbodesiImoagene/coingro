...
from coingro.data.btanalysis import (
    BT_DATA_COLUMNS,
...
def test_get_latest_backtest_filename(testdatadir, mocker):
    ...
    mocker.patch("coingro.data.btanalysis.json_load", return_value={})
...
def test_load_backtest_metadata(mocker, testdatadir):
    ...
    mocker.patch("coingro.data.btanalysis.get_backtest_metadata_filename")
    mocker.patch("coingro.data.btanalysis.json_load", side_effect=Exception())
    with pytest.raises(
...
def test_load_backtest_data_old_format(testdatadir, mocker):
    ...
    mocker.patch("coingro.data.btanalysis.load_backtest_stats", return_value=[])
...
def test_load_trades_from_db(default_conf, fee, is_short, mocker):
    ...
    trades = load_trades_from_db(
        db_url=default_conf["db_url"],
        dry_run=default_conf["dry_run"],
        strategy=CURRENT_TEST_STRATEGY,
    )
    assert len(trades) == 4
    trades = load_trades_from_db(
        db_url=default_conf["db_url"], dry_run=default_conf["dry_run"], strategy="NoneStrategy"
    )
    assert len(trades) == 0
...
def test_load_trades(default_conf, mocker):
    db_mock = mocker.patch("coingro.data.btanalysis.load_trades_from_db", MagicMock())
    bt_mock = mocker.patch("coingro.data.btanalysis.load_backtest_data", MagicMock())
