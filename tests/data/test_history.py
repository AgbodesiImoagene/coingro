...
def test_load_data_7min_timeframe(mocker, caplog, default_conf, testdatadir) -> None:
    ...
    assert log_has(
        ...
        "Use `coingro download-data` to download the data",
        caplog,
    )


def test_load_data_1min_timeframe(ohlcv_history, mocker, caplog, testdatadir) -> None:
    mocker.patch("coingro.exchange.Exchange.get_historic_ohlcv", return_value=ohlcv_history)
...
def test_load_data_mark(ohlcv_history, mocker, caplog, testdatadir) -> None:
    ...
    assert file.is_file()
    assert not log_has(
        'Download history data for pair: "UNITTEST/USDT", interval: 1m ' "and store in None.",
        caplog,
    )
...
def test_load_data_startup_candles(mocker, caplog, default_conf, testdatadir) -> None:
    ltfmock = mocker.patch(
        "coingro.data.history.jsondatahandler.JsonDataHandler._ohlcv_load",
...
def test_load_data_with_new_pair_1min(
    ohlcv_history_list, mocker, caplog, default_conf, tmpdir, candle_type
) -> None:
    """
    Test load_pair_history() with 1 min timeframe
    """
    tmpdir1 = Path(tmpdir)
    mocker.patch("coingro.exchange.Exchange.get_historic_ohlcv", return_value=ohlcv_history_list)
...
@pytest.mark.parametrize(
    ...
    [
        ("ETH/BTC", "5m", "coingro/hello/world/ETH_BTC-5m.json", ""),
        ("ETH/USDT", "1M", "coingro/hello/world/ETH_USDT-1Mo.json", ""),
        ("Fabric Token/ETH", "5m", "coingro/hello/world/Fabric_Token_ETH-5m.json", ""),
        ("ETHH20", "5m", "coingro/hello/world/ETHH20-5m.json", ""),
        (".XBTBON2H", "5m", "coingro/hello/world/_XBTBON2H-5m.json", ""),
        ("ETHUSD.d", "5m", "coingro/hello/world/ETHUSD_d-5m.json", ""),
        ("ACC_OLD/BTC", "5m", "coingro/hello/world/ACC_OLD_BTC-5m.json", ""),
        ("ETH/BTC", "5m", "coingro/hello/world/futures/ETH_BTC-5m-mark.json", "mark"),
        ("ACC_OLD/BTC", "5m", "coingro/hello/world/futures/ACC_OLD_BTC-5m-index.json", "index"),
    ],
)
def test_json_pair_data_filename(pair, timeframe, expected_result, candle_type):
    fn = JsonDataHandler._pair_data_filename(
        Path("coingro/hello/world"), pair, timeframe, CandleType.from_string(candle_type)
    )
    assert isinstance(fn, Path)
    assert fn == Path(expected_result)
    fn = JsonGzDataHandler._pair_data_filename(
        Path("coingro/hello/world"),
        pair,
        timeframe,
...
@pytest.mark.parametrize(
    ...
    [
        ("ETH/BTC", "coingro/hello/world/ETH_BTC-trades.json"),
        ("Fabric Token/ETH", "coingro/hello/world/Fabric_Token_ETH-trades.json"),
        ("ETHH20", "coingro/hello/world/ETHH20-trades.json"),
        (".XBTBON2H", "coingro/hello/world/_XBTBON2H-trades.json"),
        ("ETHUSD.d", "coingro/hello/world/ETHUSD_d-trades.json"),
        ("ACC_OLD_BTC", "coingro/hello/world/ACC_OLD_BTC-trades.json"),
    ],
)
def test_json_pair_trades_filename(pair, expected_result):
    fn = JsonDataHandler._pair_trades_filename(Path("coingro/hello/world"), pair)
    assert isinstance(fn, Path)
    assert fn == Path(expected_result)

    fn = JsonGzDataHandler._pair_trades_filename(Path("coingro/hello/world"), pair)
    assert isinstance(fn, Path)
    assert fn == Path(expected_result + ".gz")
...
def test_download_pair_history(
    ohlcv_history_list, mocker, default_conf, tmpdir, candle_type, subdir, file_tail
) -> None:
    mocker.patch("coingro.exchange.Exchange.get_historic_ohlcv", return_value=ohlcv_history_list)
    exchange = get_patched_exchange(mocker, default_conf)
    tmpdir1 = Path(tmpdir)
    ...
    assert _download_pair_history(
        datadir=tmpdir1, exchange=exchange, pair="MEME/BTC", timeframe="1m", candle_type=candle_type
    )
    assert _download_pair_history(
        datadir=tmpdir1, exchange=exchange, pair="CFI/BTC", timeframe="1m", candle_type=candle_type
    )
    ...
    assert _download_pair_history(
        datadir=tmpdir1, exchange=exchange, pair="MEME/BTC", timeframe="5m", candle_type=candle_type
    )
    assert _download_pair_history(
        datadir=tmpdir1, exchange=exchange, pair="CFI/BTC", timeframe="5m", candle_type=candle_type
    )
    assert not exchange._pairs_last_refresh_time
...
def test_download_pair_history2(mocker, default_conf, testdatadir) -> None:
    ...
    json_dump_mock = mocker.patch(
        "coingro.data.history.jsondatahandler.JsonDataHandler.ohlcv_store", return_value=None
    )
    mocker.patch("coingro.exchange.Exchange.get_historic_ohlcv", return_value=tick)
    exchange = get_patched_exchange(mocker, default_conf)
    _download_pair_history(
        datadir=testdatadir,
        exchange=exchange,
...
def test_download_backtesting_data_exception(mocker, caplog, default_conf, tmpdir) -> None:
    mocker.patch(
        "coingro.exchange.Exchange.get_historic_ohlcv", side_effect=Exception("File Error")
    )
    tmpdir1 = Path(tmpdir)
    exchange = get_patched_exchange(mocker, default_conf)

    assert not _download_pair_history(
        datadir=tmpdir1, exchange=exchange, pair="MEME/BTC", timeframe="1m", candle_type="spot"
    )
...
def test_load_partial_missing(testdatadir, caplog) -> None:
    ...
    assert log_has(
        f"UNITTEST/BTC, spot, 5m, " f'data starts at {start_real.strftime("%Y-%m-%d %H:%M:%S")}',
        caplog,
    )
    ...
    assert log_has(
        f"UNITTEST/BTC, spot, 5m, " f'data ends at {end_real.strftime("%Y-%m-%d %H:%M:%S")}', caplog
    )
...
def test_refresh_backtest_ohlcv_data(
    ...
    dl_mock = mocker.patch("coingro.data.history.history_utils._download_pair_history", MagicMock())
...
def test_refresh_backtest_trades_data(mocker, default_conf, markets, caplog, testdatadir):
    dl_mock = mocker.patch(
        "coingro.data.history.history_utils._download_trades_history", MagicMock()
    )
    mocker.patch("coingro.exchange.Exchange.markets", PropertyMock(return_value=markets))
    mocker.patch.object(Path, "exists", MagicMock(return_value=True))
    mocker.patch.object(Path, "unlink", MagicMock())
...
def test_download_trades_history(
    trades_history, mocker, default_conf, testdatadir, caplog, tmpdir
) -> None:
    tmpdir1 = Path(tmpdir)
    ght_mock = MagicMock(side_effect=lambda pair, *args, **kwargs: (pair, trades_history))
    mocker.patch("coingro.exchange.Exchange.get_historic_trades", ght_mock)
    exchange = get_patched_exchange(mocker, default_conf)
    ...
    assert _download_trades_history(data_handler=data_handler, exchange=exchange, pair="ETH/BTC")
    ...
    assert _download_trades_history(
        data_handler=data_handler, exchange=exchange, pair="ETH/BTC", timerange=timerange
    )
    ...
    mocker.patch("coingro.exchange.Exchange.get_historic_trades", MagicMock(side_effect=ValueError))
    ...
    assert _download_trades_history(
        data_handler=data_handler, exchange=exchange, pair="XRP/ETH", timerange=timerange
    )
...
def test_convert_trades_to_ohlcv(testdatadir, tmpdir, caplog):
    ...
    convert_trades_to_ohlcv(
        [pair], timeframes=["1m", "5m"], datadir=tmpdir1, timerange=tr, erase=True
    )
    ...
    convert_trades_to_ohlcv(
        ["NoDatapair"], timeframes=["1m", "5m"], datadir=tmpdir1, timerange=tr, erase=True
    )
...
def test_hdf5datahandler_ohlcv_load_and_resave(
    testdatadir, tmpdir, pair, timeframe, candle_type, candle_append, startdt, enddt
):
    tmpdir1 = Path(tmpdir)
    tmpdir2 = tmpdir1
