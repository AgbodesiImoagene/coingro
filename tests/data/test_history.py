...
def test_load_data_7min_timeframe(mocker, caplog, default_conf, testdatadir) -> None:
    ...
    assert log_has(
        ...
        'Use `coingro download-data` to download the data', caplog
    )
...
def test_download_pair_history2(mocker, default_conf, testdatadir) -> None:
    ...
    json_dump_mock = mocker.patch(
        'coingro.data.history.jsondatahandler.JsonDataHandler.ohlcv_store',
        return_value=None)
...
def test_download_backtesting_data_exception(mocker, caplog, default_conf, tmpdir) -> None:
    mocker.patch('coingro.exchange.Exchange.get_historic_ohlcv',
...
def test_refresh_backtest_ohlcv_data(
    ...
    dl_mock = mocker.patch('coingro.data.history.history_utils._download_pair_history',
                           MagicMock())
    mocker.patch(
...
def test_refresh_backtest_trades_data(mocker, default_conf, markets, caplog, testdatadir):
    dl_mock = mocker.patch('coingro.data.history.history_utils._download_trades_history',
                           MagicMock())
    mocker.patch(
        'coingro.exchange.Exchange.markets', PropertyMock(return_value=markets)
    )
...
def test_download_trades_history(trades_history, mocker, default_conf, testdatadir, caplog,
    ...
    mocker.patch('coingro.exchange.Exchange.get_historic_trades',
                 ght_mock)
    exchange = get_patched_exchange(mocker, default_conf)
