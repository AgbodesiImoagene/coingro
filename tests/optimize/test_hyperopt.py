...
from coingro.commands.optimize_commands import setup_optimize_configuration, start_hyperopt
...
from coingro.optimize.hyperopt import Hyperopt
from coingro.optimize.hyperopt_auto import HyperOptAuto
from coingro.optimize.hyperopt_tools import HyperoptTools
from coingro.optimize.optimize_reports import generate_strategy_stats
from coingro.optimize.space import SKDecimal
...
def test_start_not_installed(mocker, default_conf, import_fails) -> None:
    ...
    mocker.patch('coingro.optimize.hyperopt.Hyperopt.start', start_mock)
...
def test_start_no_data(mocker, hyperopt_conf, tmpdir) -> None:
    ...
    patched_configuration_load_config_file(mocker, hyperopt_conf)
    mocker.patch('coingro.data.history.load_pair_history', MagicMock(return_value=pd.DataFrame))
    mocker.patch(
        'coingro.optimize.hyperopt.get_timerange',
...
def test_start_filelock(mocker, hyperopt_conf, caplog) -> None:
    ...
    patched_configuration_load_config_file(mocker, hyperopt_conf)
    mocker.patch('coingro.optimize.hyperopt.Hyperopt.__init__', hyperopt_mock)
    patch_exchange(mocker)
    ...
    pargs = get_args(args)
    start_hyperopt(pargs)
    assert log_has("Another running instance of coingro Hyperopt detected.", caplog)
...
def test_start_calls_optimizer(mocker, hyperopt_conf, capsys) -> None:
    dumper = mocker.patch('coingro.optimize.hyperopt.dump')
    dumper2 = mocker.patch('coingro.optimize.hyperopt.Hyperopt._save_result')
    mocker.patch('coingro.optimize.hyperopt.file_dump_json')

    mocker.patch('coingro.optimize.backtesting.Backtesting.load_bt_data',
    ...
    parallel = mocker.patch(
        'coingro.optimize.hyperopt.Hyperopt.run_optimizer_parallel',
...
def test_generate_optimizer(mocker, hyperopt_conf) -> None:
    ...
    mocker.patch('coingro.optimize.hyperopt.Backtesting.backtest', return_value=backtest_result)
    mocker.patch('coingro.optimize.hyperopt.get_timerange',
                 return_value=(Arrow(2017, 12, 10), Arrow(2017, 12, 13)))
    patch_exchange(mocker)
    mocker.patch.object(Path, 'open')
    mocker.patch('coingro.configuration.config_validation.validate_config_schema')
    mocker.patch('coingro.optimize.hyperopt.load', return_value={'XRP/BTC': None})
...
def test_clean_hyperopt(mocker, hyperopt_conf, caplog):
    ...
    mocker.patch("coingro.strategy.hyper.HyperStrategyMixin.load_params_from_file",
                 MagicMock(return_value={}))
    mocker.patch("coingro.optimize.hyperopt.Path.is_file", MagicMock(return_value=True))
    unlinkmock = mocker.patch("coingro.optimize.hyperopt.Path.unlink", MagicMock())
...
def test_simplified_interface_all_failed(mocker, hyperopt_conf, caplog) -> None:
    mocker.patch('coingro.optimize.hyperopt.dump', MagicMock())
    mocker.patch('coingro.optimize.hyperopt.file_dump_json')
    mocker.patch('coingro.optimize.backtesting.Backtesting.load_bt_data',
                 MagicMock(return_value=(MagicMock(), None)))
    mocker.patch(
    ...
    mocker.patch('coingro.optimize.hyperopt_auto.HyperOptAuto._generate_indicator_space',
                 return_value=[])
