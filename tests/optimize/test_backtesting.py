...
def test_setup_bt_configuration_with_arguments(mocker, default_conf, caplog) -> None:
    patched_configuration_load_config_file(mocker, default_conf)
    mocker.patch(
        'coingro.configuration.configuration.create_datadir',
        lambda c, x: x
    )
...
def test_backtesting_start(default_conf, mocker, testdatadir, caplog) -> None:
    ...
    mocker.patch('coingro.plugins.pairlistmanager.PairListManager.whitelist',
...
def test_backtesting_start_no_data(default_conf, mocker, caplog, testdatadir) -> None:
    ...
    mocker.patch('coingro.data.history.history_utils.load_pair_history',
...
def test_backtest__enter_trade_futures(default_conf_usdt, fee, mocker) -> None:
    ...
    mocker.patch("coingro.exchange.Exchange.get_maintenance_ratio_and_amt",
    ...
    mocker.patch("coingro.wallets.Wallets.get_trade_stake_amount",
...
def test_backtest_one(default_conf, fee, mocker, testdatadir) -> None:
    ...
    expected = pd.DataFrame(
        {'pair': [pair, pair],
         ...
         'orders': [
            [
                {'amount': 0.00957442, 'safe_price': 0.104445, 'cg_order_side': 'buy',
                 'order_filled_timestamp': 1517251200000, 'cg_is_entry': True},
                {'amount': 0.00957442, 'safe_price': 0.10496853383458644, 'cg_order_side': 'sell',
                 'order_filled_timestamp': 1517265300000, 'cg_is_entry': False}
            ], [
                {'amount': 0.0097064, 'safe_price': 0.10302485, 'cg_order_side': 'buy',
                 'order_filled_timestamp': 1517283000000, 'cg_is_entry': True},
                {'amount': 0.0097064, 'safe_price': 0.10354126528822055, 'cg_order_side': 'sell',
                 'order_filled_timestamp': 1517285400000, 'cg_is_entry': False}
            ]
         ]
         })
...
def test_backtest_start_multi_strat(default_conf, mocker, caplog, testdatadir):
    ...
    mocker.patch.multiple('coingro.optimize.optimize_reports',
...
def test_backtest_start_multi_strat_caching(default_conf, mocker, caplog, testdatadir, run_id,
    ...
    mocker.patch.multiple('coingro.data.btanalysis',
