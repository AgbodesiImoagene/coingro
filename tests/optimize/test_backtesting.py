...
from coingro.commands.optimize_commands import setup_optimize_configuration, start_backtesting
from coingro.configuration import TimeRange
from coingro.data import history
from coingro.data.btanalysis import BT_DATA_COLUMNS, evaluate_result_multi
from coingro.data.converter import clean_ohlcv_dataframe
from coingro.data.dataprovider import DataProvider
from coingro.data.history import get_timerange
from coingro.enums import ExitType, RunMode
...
def load_data_test(what, testdatadir):
    ...
    return {
        "UNITTEST/BTC": clean_ohlcv_dataframe(
            data, timeframe="1m", pair="UNITTEST/BTC", fill_missing=True
        )
    }
...
def test_setup_bt_configuration_with_arguments(mocker, default_conf, caplog) -> None:
    patched_configuration_load_config_file(mocker, default_conf)
    mocker.patch("coingro.configuration.configuration.create_datadir", lambda c, x: x)
...
def test_start(mocker, fee, default_conf, caplog) -> None:
    ...
    patch_exchange(mocker)
    mocker.patch("coingro.optimize.backtesting.Backtesting.start", start_mock)
    patched_configuration_load_config_file(mocker, default_conf)
    ...
    assert log_has("Starting coingro in Backtesting mode", caplog)
...
def test_backtesting_init(mocker, default_conf, order_types) -> None:
    ...
    patch_exchange(mocker)
    get_fee = mocker.patch("coingro.exchange.Exchange.get_fee", MagicMock(return_value=0.5))
...
def test_backtesting_init_no_timeframe(mocker, default_conf, caplog) -> None:
    ...
    mocker.patch("coingro.exchange.Exchange.get_fee", MagicMock(return_value=0.5))
    with pytest.raises(
...
def test_data_with_fee(default_conf, mocker) -> None:
    ...
    fee_mock = mocker.patch("coingro.exchange.Exchange.get_fee", MagicMock(return_value=0.5))
    backtesting = Backtesting(default_conf)
    backtesting._set_strategy(backtesting.strategylist[0])
...
def test_backtesting_start(default_conf, mocker, testdatadir, caplog) -> None:
    ...
    mocker.patch("coingro.data.history.get_timerange", get_timerange)
    patch_exchange(mocker)
    mocker.patch("coingro.optimize.backtesting.Backtesting.backtest")
    mocker.patch("coingro.optimize.backtesting.generate_backtest_stats")
    mocker.patch("coingro.optimize.backtesting.show_backtest_results")
    sbs = mocker.patch("coingro.optimize.backtesting.store_backtest_stats")
    sbc = mocker.patch("coingro.optimize.backtesting.store_backtest_signal_candles")
    mocker.patch(
        "coingro.plugins.pairlistmanager.PairListManager.whitelist",
...
def test_backtesting_start_no_data(default_conf, mocker, caplog, testdatadir) -> None:
    ...
    mocker.patch(
        "coingro.data.history.history_utils.load_pair_history",
...
def test_backtesting_no_pair_left(default_conf, mocker, caplog, testdatadir) -> None:
    ...
    mocker.patch("coingro.optimize.backtesting.Backtesting.backtest")
    mocker.patch(
        "coingro.plugins.pairlistmanager.PairListManager.whitelist", PropertyMock(return_value=[])
    )
...
def test_backtesting_pairlist_list(default_conf, mocker, caplog, testdatadir, tickers) -> None:
    ...
    mocker.patch("coingro.plugins.pairlistmanager.PairListManager.refresh_pairlist")
...
def test_backtest__enter_trade(default_conf, fee, mocker) -> None:
    ...
    mocker.patch("coingro.exchange.Exchange.get_fee", fee)
    mocker.patch("coingro.exchange.Exchange.get_min_pair_stake_amount", return_value=0.00001)
    mocker.patch("coingro.exchange.Exchange.get_max_pair_stake_amount", return_value=float("inf"))
    patch_exchange(mocker)
    ...
    mocker.patch("coingro.exchange.Exchange.get_max_pair_stake_amount", return_value=300.0)
    trade = backtesting._enter_trade(pair, row=row, direction="long")
    assert trade
...
def test_backtest__enter_trade_futures(default_conf_usdt, fee, mocker) -> None:
    ...
    mocker.patch("coingro.exchange.Exchange.get_min_pair_stake_amount", return_value=0.00001)
    mocker.patch("coingro.exchange.Exchange.get_max_pair_stake_amount", return_value=float("inf"))
    mocker.patch("coingro.exchange.Exchange.get_max_leverage", return_value=100)
    patch_exchange(mocker)
    ...
    mocker.patch(
        "coingro.exchange.Exchange.get_maintenance_ratio_and_amt", return_value=(0.01, 0.01)
    )
    ...
    # coingro_liquidation_price = liq + (abs(open_rate - liq) * liq_buffer * side_1)
    ...
    mocker.patch("coingro.exchange.Exchange.get_min_pair_stake_amount", return_value=600.0)

    trade = backtesting._enter_trade(pair, row=row, direction="long")
    assert trade is None

    # Stake-amount throwing error
    mocker.patch("coingro.wallets.Wallets.get_trade_stake_amount", side_effect=DependencyException)
    ...
    expected = pd.DataFrame(
        {
            ...
            "open_date": pd.to_datetime(
                [Arrow(2018, 1, 29, 18, 40, 0).datetime, Arrow(2018, 1, 30, 3, 30, 0).datetime],
                utc=True,
            ),
            "close_date": pd.to_datetime(
                [Arrow(2018, 1, 29, 22, 35, 0).datetime, Arrow(2018, 1, 30, 4, 10, 0).datetime],
                utc=True,
            ),
...
def test_backtest_dataprovider_analyzed_df(default_conf, fee, mocker, testdatadir) -> None:
    ...
    mocker.patch("coingro.exchange.Exchange.get_fee", fee)
    mocker.patch("coingro.exchange.Exchange.get_min_pair_stake_amount", return_value=0.00001)
    mocker.patch("coingro.exchange.Exchange.get_max_pair_stake_amount", return_value=100000)
    patch_exchange(mocker)
    backtesting = Backtesting(default_conf)
    backtesting._set_strategy(backtesting.strategylist[0])
    timerange = TimeRange("date", None, 1517227800, 0)
...
def test_backtest_pricecontours(
    default_conf, fee, mocker, testdatadir, protections, contour, expected
) -> None:
    if protections:
...
def test_backtest_start_timerange(default_conf, mocker, caplog, testdatadir):
    ...
    exists = [
        ...
        "Loading data from 2017-11-14 20:57:00 " "up to 2017-11-14 22:58:00 (0 days).",
...
def test_backtest_start_multi_strat(default_conf, mocker, caplog, testdatadir):
    ...
    mocker.patch("coingro.optimize.backtesting.Backtesting.backtest", backtestmock)
    ...
    mocker.patch.multiple(
        "coingro.optimize.optimize_reports",
...
def test_backtest_start_nomock_futures(default_conf_usdt, mocker, caplog, testdatadir, capsys):
    ...
    result2 = pd.DataFrame(
        {
            "pair": ["XRP/USDT", "XRP/USDT", "XRP/USDT"],
            "profit_ratio": [0.03, 0.01, 0.1],
            "profit_abs": [0.01, 0.02, 0.2],
            "open_date": pd.to_datetime(
    ...
    exists = [
        ...
        f"Using data directory: {testdatadir} ...",
        "Loading data from 2021-11-17 01:00:00 " "up to 2021-11-21 03:00:00 (4 days).",
        "Backtesting with data from 2021-11-17 21:00:00 " "up to 2021-11-21 03:00:00 (3 days).",
...
def test_backtest_start_multi_strat_nomock_detail(
    ...
    exists = [
        ...
        f"Using data directory: {testdatadir} ...",
        "Loading data from 2019-10-11 00:00:00 " "up to 2019-10-13 11:10:00 (2 days).",
        "Backtesting with data from 2019-10-11 01:40:00 " "up to 2019-10-13 11:10:00 (2 days).",
        f"Running backtesting for Strategy {CURRENT_TEST_STRATEGY}",
    ]
...
def test_backtest_start_multi_strat_caching(
    ...
    mocker.patch("coingro.optimize.backtesting.Backtesting.backtest", backtestmock)
    mocker.patch("coingro.optimize.backtesting.show_backtest_results", MagicMock())
    ...
    mocker.patch.multiple(
        "coingro.data.btanalysis",
        load_backtest_metadata=load_backtest_metadata,
        load_backtest_stats=load_backtest_stats,
    )
    mocker.patch("coingro.optimize.backtesting.get_strategy_run_id", side_effect=["1", "2", "2"])
