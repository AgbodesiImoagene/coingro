...
from coingro.persistence.models import init_db
...
def test_start_trading_fail(mocker, caplog):
    ...
    mocker.patch("coingro.worker.Worker.run", MagicMock(side_effect=OperationalException))

    mocker.patch("coingro.worker.Worker.__init__", MagicMock(return_value=None))

    exitmock = mocker.patch("coingro.worker.Worker.exit", MagicMock())
    ...
    exitmock.reset_mock()
    caplog.clear()
    mocker.patch("coingro.worker.Worker.__init__", MagicMock(side_effect=OperationalException))
    start_trading(get_args(args))
...
def test_start_webserver(mocker, caplog):
    ...
    api_server_mock = mocker.patch(
        "coingro.rpc.api_server.ApiServer",
    )

    args = ["webserver", "-c", "config_examples/config_bittrex.example.json"]
...
def test_list_exchanges(capsys):
    ...
    assert re.match(r"Exchanges available for Coingro.*", captured.out)
...
def test_list_timeframes(mocker, capsys):
    ...
    captured = capsys.readouterr()
    assert re.match(
        "Timeframes available for the exchange `Bittrex`: " "1m, 5m, 30m, 1h, 1d", captured.out
    )
...
def test_list_markets(mocker, markets_static, capsys):
    ...
    captured = capsys.readouterr()
    assert (
        "Exchange Bittrex has 12 active markets: "
    ...
    captured = capsys.readouterr()
    assert (
        "Exchange Bittrex has 14 markets: "
    ...
    captured = capsys.readouterr()
    assert (
        "Exchange Bittrex has 7 active markets with ETH, LTC as base currencies: "
    ...
    captured = capsys.readouterr()
    assert (
        "Exchange Bittrex has 4 active markets with LTC as base currency: "
    ...
    captured = capsys.readouterr()
    assert (
        "Exchange Bittrex has 5 active markets with USDT, USD as quote currencies: "
    ...
    captured = capsys.readouterr()
    assert (
        "Exchange Bittrex has 4 active markets with USDT as quote currency: "
    ...
    captured = capsys.readouterr()
    assert (
        "Exchange Bittrex has 1 active pair with LTC as base currency and "
    ...
    mocker.patch("coingro.exchange.Exchange.markets", PropertyMock(side_effect=ValueError))
    # Test --one-column
    args = [
        "list-markets",
        "--config",
...
def test_create_datadir(caplog, mocker):
    ...
    cud = mocker.patch("coingro.commands.deploy_commands.create_userdata_dir", MagicMock())
    csf = mocker.patch("coingro.commands.deploy_commands.copy_sample_files", MagicMock())
    args = ["create-userdir", "--userdir", "/temp/coingro/test"]
    start_create_userdir(get_args(args))
...
def test_start_new_strategy(mocker, caplog):
    ...
    mocker.patch("coingro.commands.deploy_commands.setup_utils_configuration")
...
def test_clean_ui_subdir(mocker, tmpdir, caplog):
    mocker.patch("coingro.commands.deploy_commands.Path.is_dir", side_effect=[True, True])
    mocker.patch("coingro.commands.deploy_commands.Path.is_file", side_effect=[False, True])
    rd_mock = mocker.patch("coingro.commands.deploy_commands.Path.rmdir")
    ul_mock = mocker.patch("coingro.commands.deploy_commands.Path.unlink")

    mocker.patch(
        "coingro.commands.deploy_commands.Path.glob",
...
def test_download_and_install_ui(mocker, tmpdir):
    ...
    mocker.patch("coingro.commands.deploy_commands.requests.get", return_value=requests_mock)

    mocker.patch("coingro.commands.deploy_commands.Path.is_dir", side_effect=[True, False])
    wb_mock = mocker.patch("coingro.commands.deploy_commands.Path.write_bytes")
...
def test_get_ui_download_url(mocker):
    ...
    get_mock = mocker.patch("coingro.commands.deploy_commands.requests.get", return_value=response)
...
def test_download_data_keyboardInterrupt(mocker, caplog, markets):
    dl_mock = mocker.patch(
        "coingro.commands.data_commands.refresh_backtest_ohlcv_data",
...
def test_download_data_timerange(mocker, caplog, markets):
    ...
    assert (
        dl_mock.call_args_list[0][1]["timerange"].startts == arrow.Arrow(2020, 1, 1).int_timestamp
    )
...
def test_download_data_trades(mocker, caplog):
    dl_mock = mocker.patch(
        "coingro.commands.data_commands.refresh_backtest_trades_data", MagicMock(return_value=[])
    )
    convert_mock = mocker.patch(
        "coingro.commands.data_commands.convert_trades_to_ohlcv", MagicMock(return_value=[])
    )
    patch_exchange(mocker)
    mocker.patch("coingro.exchange.Exchange.markets", PropertyMock(return_value={}))
    args = [
...
def test_start_test_pairlist(mocker, caplog, tickers, default_conf, capsys):
    ...
    args = ["test-pairlist", "-c", "config_examples/config_bittrex.example.json"]
...
def test_hyperopt_list(mocker, capsys, caplog, saved_hyperopt_results, tmpdir):
    ...
    mocker.patch(
        "coingro.optimize.hyperopt_tools.HyperoptTools._test_hyperopt_results_exist",
        return_value=True,
    )
    ...
    mocker.patch(
        "coingro.optimize.hyperopt_tools.HyperoptTools._read_results", side_effect=fake_iterator
    )
    ...
    assert (
        'Best,1,2,-1.25%,-1.2222,-0.00125625,BTC,-2.51,"3,930.0 m",0.43662' in line
...
def test_hyperopt_show(mocker, capsys, saved_hyperopt_results):
    ...
    mocker.patch("coingro.commands.hyperopt_commands.show_backtest_result")
...
def test_convert_data(mocker, testdatadir):
    ohlcv_mock = mocker.patch("coingro.commands.data_commands.convert_ohlcv_format")
    trades_mock = mocker.patch("coingro.commands.data_commands.convert_trades_format")
    args = [
        "convert-data",
        "--format-from",
        "json",
...
def test_show_trades(mocker, fee, capsys, caplog):
    mocker.patch("coingro.persistence.init_db")
...
def test_backtesting_show(mocker, testdatadir, capsys):
    sbr = mocker.patch("coingro.optimize.optimize_reports.show_backtest_results")
    args = [
        "backtesting-show",
        "--export-filename",
