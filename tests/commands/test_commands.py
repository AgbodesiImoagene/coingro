...
def test_start_webserver(mocker, caplog):
    ...
    api_server_mock = mocker.patch("coingro.rpc.api_server.ApiServer", )
...
def test_clean_ui_subdir(mocker, tmpdir, caplog):
    mocker.patch("coingro.commands.deploy_commands.Path.is_dir",
                 side_effect=[True, True])
    mocker.patch("coingro.commands.deploy_commands.Path.is_file",
    ...
    mocker.patch("coingro.commands.deploy_commands.Path.glob",
...
def test_get_ui_download_url(mocker):
    ...
    get_mock = mocker.patch("coingro.commands.deploy_commands.requests.get",
...
def test_download_data_keyboardInterrupt(mocker, caplog, markets):
    dl_mock = mocker.patch('coingro.commands.data_commands.refresh_backtest_ohlcv_data',
                           MagicMock(side_effect=KeyboardInterrupt))
    patch_exchange(mocker)
    mocker.patch(
        'coingro.exchange.Exchange.markets', PropertyMock(return_value=markets)
    )
    args = [
        "download-data",
        "--exchange", "binance",
...
def test_download_data_no_markets(mocker, caplog):
    ...
    mocker.patch(
        'coingro.exchange.Exchange.markets', PropertyMock(return_value={})
    )
    args = [
        "download-data",
        "--exchange", "binance",
...
def test_download_data_no_exchange(mocker, caplog):
    mocker.patch('coingro.commands.data_commands.refresh_backtest_ohlcv_data',
                 MagicMock(return_value=["ETH/BTC", "XRP/BTC"]))
    patch_exchange(mocker)
    mocker.patch(
        'coingro.exchange.Exchange.markets', PropertyMock(return_value={})
...
def test_download_data_trades(mocker, caplog):
    dl_mock = mocker.patch('coingro.commands.data_commands.refresh_backtest_trades_data',
                           MagicMock(return_value=[]))
    convert_mock = mocker.patch('coingro.commands.data_commands.convert_trades_to_ohlcv',
                                MagicMock(return_value=[]))
    patch_exchange(mocker)
    mocker.patch(
        'coingro.exchange.Exchange.markets', PropertyMock(return_value={})
...
def test_hyperopt_list(mocker, capsys, caplog, saved_hyperopt_results, tmpdir):
    ...
    mocker.patch(
        'coingro.optimize.hyperopt_tools.HyperoptTools._read_results',
