...
from coingro.configuration.deprecated_settings import (
...
from coingro.configuration.load_config import (
    load_config_file,
    load_file,
    load_from_files,
...
def test_load_config_combine_dicts(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch("coingro.configuration.load_config.load_config_file", configsmock)
...
def test_print_config(default_conf, mocker, caplog) -> None:
    ...
    configsmock = MagicMock(side_effect=config_files)
    mocker.patch("coingro.configuration.configuration.create_datadir", lambda c, x: x)
    mocker.patch("coingro.configuration.configuration.load_from_files", configsmock)
...
def test_load_config_file_exception(mocker) -> None:
    mocker.patch(
        "coingro.configuration.configuration.open",
...
def test_setup_configuration_with_arguments(mocker, default_conf, caplog) -> None:
    ...
    mocker.patch("coingro.configuration.configuration.create_datadir", lambda c, x: x)
    mocker.patch(
        "coingro.configuration.configuration.create_userdata_dir",
...
def test_check_exchange(default_conf, caplog) -> None:
    ...
    assert log_has_re(
        r"Exchange .* is officially supported by the Coingro development team\.", caplog
    )
    caplog.clear()
    ...
    assert log_has_re(
        ...
        r"by the Coingro development team\. .*",
        caplog,
    )
    caplog.clear()
    ...
    with pytest.raises(OperationalException, match=r"Exchange .* will not work with Coingro\..*"):
...
def test_cli_verbose_with_params(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch("coingro.loggers._set_loggers", MagicMock)
    arglist = ["trade", "-vvv"]
    args = Arguments(arglist).get_parsed_arg()
...
def test_pairlist_resolving_fallback(mocker):
    ...
    mocker.patch(
        "coingro.configuration.configuration.load_file",
...
def test_flat_vars_to_nested_dict(caplog):
    ...
    test_args = {
        "COINGRO__EXCHANGE__SOME_SETTING": "true",
        "COINGRO__EXCHANGE__SOME_FALSE_SETTING": "false",
        "COINGRO__EXCHANGE__CONFIG__whatever": "sometime",
        "COINGRO__EXIT_PRICING__PRICE_SIDE": "bid",
        "COINGRO__EXIT_PRICING__cccc": "500",
        "COINGRO__STAKE_AMOUNT": "200.05",
        "COINGRO__TELEGRAM__CHAT_ID": "2151",
