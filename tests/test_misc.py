...
def test_file_dump_json(mocker) -> None:
    file_open = mocker.patch("coingro.misc.open", MagicMock())
    json_dump = mocker.patch("rapidjson.dump", MagicMock())
    file_dump_json(Path("somefile"), [1, 2, 3])
    assert file_open.call_count == 1
    assert json_dump.call_count == 1
    file_open = mocker.patch("coingro.misc.gzip.open", MagicMock())
    json_dump = mocker.patch("rapidjson.dump", MagicMock())
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "sqlite:////coingro/user_data/tradesv3.sqlite",
            "sqlite:////coingro/user_data/tradesv3.sqlite",
        ),
    ],
)
...
def test_retrier(caplog):
    ...
    assert (
        num_log_has_re(
            (r'test_funct_a\(\) returned exception: ".*". ' r"Retrying still for \d* times."),
            caplog,
        )
        == 0
    )
    ...
    assert (
        num_log_has_re(
            (r'test_funct_b\(\) returned exception: ".*". ' r"Retrying still for \d* times."),
            caplog,
        )
        == 5
    )
    ...
    assert (
        num_log_has_re(
            (r'test_funct_c\(\) returned exception: ".*". ' r"Retrying still for \d* times."),
            caplog,
        )
        == 4
    )
