...
from coingro.rpc.api_server.api_auth import create_token, get_user_from_token
...
def botclient(default_conf, mocker):
    ...
    rpc = RPC(cgbot)
    mocker.patch("coingro.rpc.api_server.ApiServer.start_api", MagicMock())
    try:
        apiserver = ApiServer(default_conf)
        apiserver.add_rpc_handler(rpc)
        yield cgbot, TestClient(apiserver.app)
        # Cleanup ... ?
    finally:
        ApiServer.shutdown()
...
def test_api_auth():
    with pytest.raises(ValueError):
        create_token({"identity": {"u": "Coingro"}}, "secret1234", token_type="NotATokenType")

    token = create_token({"identity": {"u": "Coingro"}}, "secret1234")
    ...
    token = create_token({"identity": {"u1": "Coingro"}}, "secret1234")
    with pytest.raises(HTTPException):
...
def test_api_unauthorized(botclient):
    ...
    cgbot.config["api_server"]["username"] = "WrongUser"
    rc = client_get(client, f"{BASE_URI}/version")
    assert_response(rc, 401)
    assert rc.json() == {"detail": "Unauthorized"}

    # Change only password
    cgbot.config["api_server"]["username"] = _TEST_USER
    cgbot.config["api_server"]["password"] = "WrongPassword"
    rc = client_get(client, f"{BASE_URI}/version")
    assert_response(rc, 401)
    assert rc.json() == {"detail": "Unauthorized"}
...
def test_api__init__(default_conf, mocker):
    ...
    mocker.patch("coingro.rpc.telegram.Updater", MagicMock())
    mocker.patch("coingro.rpc.api_server.webserver.ApiServer.start_api", MagicMock())
    apiserver = ApiServer(default_conf)
    apiserver.add_rpc_handler(RPC(get_patched_coingrobot(mocker, default_conf)))
...
def test_api_UvicornServer(mocker):
    thread_mock = mocker.patch("coingro.rpc.api_server.uvicorn_threaded.threading.Thread")
...
def test_api_UvicornServer_run(mocker):
    serve_mock = mocker.patch(
        "coingro.rpc.api_server.uvicorn_threaded.UvicornServer.serve", get_mock_coro(None)
    )
    s = UvicornServer(uvicorn.Config(MagicMock(), port=8080, host="127.0.0.1"))
...
def test_api_run(default_conf, mocker, caplog):
    ...
    mocker.patch("coingro.rpc.api_server.webserver.UvicornServer", server_mock)
    ...
    caplog.clear()
    mocker.patch("coingro.rpc.api_server.webserver.UvicornServer", MagicMock(side_effect=Exception))
    apiserver.start_api()
...
def test_api_reloadconf(botclient):
    ...
    assert cgbot.state == State.RELOAD_CONFIG


def test_api_stopbuy(botclient):
    cgbot, client = botclient
    assert cgbot.config["max_open_trades"] != 0
...
def test_api_balance(botclient, mocker, rpc_balance, tickers):
    ...
    cgbot.config["dry_run"] = False
    mocker.patch("coingro.exchange.Exchange.get_balances", return_value=rpc_balance)
    mocker.patch("coingro.exchange.Exchange.get_tickers", tickers)
    mocker.patch(
        "coingro.exchange.Exchange.get_valid_pair_combination", side_effect=lambda a, b: f"{a}/{b}"
    )
    cgbot.wallets.update()
...
def test_api_count(botclient, mocker, ticker, fee, markets, is_short):
    ...
    cgbot.config["max_open_trades"] = float("inf")
    rc = client_get(client, f"{BASE_URI}/count")
    assert rc.json()["max"] == -1
...
def test_api_show_config(botclient):
    ...
    assert response["bot_name"] == "coingro"
...
def test_api_trade_single(botclient, mocker, fee, ticker, markets, is_short):
    cgbot, client = botclient
    patch_get_signal(cgbot, enter_long=not is_short, enter_short=is_short)
    mocker.patch.multiple(
        "coingro.exchange.Exchange",
        markets=PropertyMock(return_value=markets),
        fetch_ticker=ticker,
...
def test_api_delete_trade(botclient, mocker, fee, markets, is_short):
    ...
    cgbot.strategy.order_types["stoploss_on_exchange"] = True
    ...
    [
        (
            True,
            {
                ...
                "profit_all_fiat": 538398.67323435,
                "profit_all_percent_mean": 66.41,
                "profit_all_ratio_mean": 0.664109545,
                "profit_all_percent_sum": 398.47,
                "profit_all_ratio_sum": 3.98465727,
                "profit_all_percent": 4.36,
                "profit_all_ratio": 0.043612222872799825,
        ...
        (
            None,
            {
                ...
                "profit_all_fiat": -178235.92673175,
                "profit_all_percent_mean": 0.08,
                "profit_all_ratio_mean": 0.000835751666666662,
                "profit_all_percent_sum": 0.5,
                "profit_all_ratio_sum": 0.005014509999999972,
                "profit_all_percent": -1.44,
                "profit_all_ratio": -0.014437768014451796,
...
def test_api_status(
    botclient, mocker, ticker, fee, markets, is_short, current_rate, open_order_id, open_trade_value
):
    cgbot, client = botclient
    patch_get_signal(cgbot)
    mocker.patch.multiple(
...
def test_api_force_entry(botclient, mocker, fee, endpoint):
    ...
    # enable forcebuy
    cgbot.config["force_entry_enable"] = True
...
def test_api_pair_candles(botclient, ohlcv_history):
    ...
    cgbot.dataprovider._set_cached_df("XRP/BTC", timeframe, ohlcv_history, CandleType.SPOT)
...
def test_api_pair_history(botclient, ohlcv_history):
    ...
    assert_response(rc, 502)
    assert rc.json()["error"] == (
        "Error querying /api/v1/pair_history: "
...
def test_api_plot_config(botclient):
    ...
    cgbot.strategy.plot_config = {
        "main_plot": {"sma": {}},
        "subplots": {"RSI": {"rsi": {"color": "red"}}},
    }
    rc = client_get(client, f"{BASE_URI}/plot_config")
    assert_response(rc)
    assert rc.json() == cgbot.strategy.plot_config
    assert isinstance(rc.json()["main_plot"], dict)
    assert isinstance(rc.json()["subplots"], dict)

    cgbot.strategy.plot_config = {"main_plot": {"sma": {}}}
    rc = client_get(client, f"{BASE_URI}/plot_config")
    assert_response(rc)
...
def test_list_available_pairs(botclient):
    ...
    cgbot.config["trading_mode"] = "futures"
...
#     cgbot, client = botclient
#     mocker.patch("coingro.exchange.Exchange.get_fee", fee)

#     rc = client_get(client, f"{BASE_URI}/backtest")
#     # Backtest prevented in default mode
#     assert_response(rc, 502)

#     cgbot.config["runmode"] = RunMode.WEBSERVER
...
#     cgbot.config["export"] = "trades"
#     cgbot.config["backtest_cache"] = "day"
#     cgbot.config["user_data_dir"] = Path(tmpdir)
#     cgbot.config["exportfilename"] = Path(tmpdir) / "backtest_results"
#     cgbot.config["exportfilename"].mkdir()
...
#     mocker.patch(
#         "coingro.optimize.backtesting.Backtesting.backtest_one_strategy",
...
#     cgbot, client = botclient
#     mocker.patch(
#         "coingro.data.btanalysis._get_backtest_files",
#         return_value=[
...
#     assert_response(rc, 502)
#     cgbot.config["user_data_dir"] = testdatadir
#     cgbot.config["runmode"] = RunMode.WEBSERVER
...
#     rc = client_get(
#         client,
#         f"{BASE_URI}/backtest/history/result?filename={fn}&strategy={strategy}"
#     )
#     assert_response(rc)
#     result2 = rc.json()
#     assert result2
...
def test_update_exchange(botclient, mocker, default_conf):
    ...
    rc = client_post(client, f"{BASE_URI}/exchange", data='{"name": "abcd"}')
    assert_response(rc, 502)
    assert rc.json() == {
        "error": "Error querying /api/v1/exchange: " "abcd is not a supported exchange."
    }

    rc = client_post(client, f"{BASE_URI}/exchange", data='{"name": "binance", "dry_run": true}')
    print(rc.json())
    assert_response(rc)
    assert rc.json() == {
...
def test_update_strategy(botclient, mocker, default_conf):
    ...
    rc = client_post(client, f"{BASE_URI}/strategy", data='{"strategy": "SampleStrategy"}')
    assert_response(rc)
    assert rc.json() == {
        "status": "Successfully updated config. " "Reload config for changes to take effect."
    }
...
def test_update_settings(botclient, mocker, default_conf):
    ...
    rc = client_post(client, f"{BASE_URI}/settings", data='{"max_open_trades": -4}')
    assert_response(rc, 422)
    assert rc.json()["detail"][0]["msg"] == "ensure this value is greater than or equal to -1"

    rc = client_post(client, f"{BASE_URI}/settings", data='{"stake_currency": "BTC"}')
    assert_response(rc)
    assert rc.json() == {
        "status": "Successfully updated config. " "Reload config for changes to take effect."
    }
