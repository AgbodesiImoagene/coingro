...
from tests.conftest import (CURRENT_TEST_STRATEGY, create_mock_trades, get_mock_coro,
                            get_patched_coingrobot, log_has, log_has_re, patch_get_signal)
...
def test_api_UvicornServer_run(mocker):
    serve_mock = mocker.patch('coingro.rpc.api_server.uvicorn_threaded.UvicornServer.serve',
                              get_mock_coro(None))
    s = UvicornServer(uvicorn.Config(MagicMock(), port=8080, host='127.0.0.1'))
...
def test_api_run(default_conf, mocker, caplog):
    ...
    caplog.clear()
    mocker.patch('coingro.rpc.api_server.webserver.UvicornServer',
...
def test_api_status(botclient, mocker, ticker, fee, markets, is_short,
    ...
    mocker.patch('coingro.exchange.Exchange.get_rate',
...
def test_api_backtesting(botclient, mocker, fee, caplog, tmpdir):
    ...
    mocker.patch('coingro.optimize.backtesting.Backtesting.backtest_one_strategy',
...
def test_api_backtest_history(botclient, mocker, testdatadir):
    cgbot, client = botclient
    mocker.patch('coingro.data.btanalysis._get_backtest_files',
                 return_value=[
