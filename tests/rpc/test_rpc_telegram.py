...
from tests.conftest import (CURRENT_TEST_STRATEGY, create_mock_trades, create_mock_trades_usdt,
                            get_patched_coingrobot, log_has, log_has_re, patch_exchange,
...
def test_daily_handle(default_conf_usdt, update, ticker, fee, mocker, time_machine) -> None:
    mocker.patch(
        'coingro.rpc.rpc.CryptoToFiatConverter._find_price',
        return_value=1.1
    )
    mocker.patch.multiple(
        'coingro.exchange.Exchange',
...
def test_telegram_balance_handle(default_conf, update, mocker, rpc_balance, tickers) -> None:
    ...
    mocker.patch('coingro.exchange.Exchange.get_valid_pair_combination',
...
def test_balance_handle_too_large_response(default_conf, update, mocker) -> None:
    ...
    mocker.patch('coingro.rpc.rpc.RPC._rpc_balance', return_value={
        'currencies': balances,
        'total': 100.0,
        'symbol': 100.0,
        'value': 1000.0,
...
def test_telegram_force_exit_down_handle(default_conf, update, ticker, fee,
    ...
    mocker.patch('coingro.rpc.fiat_convert.CryptoToFiatConverter._find_price',
...
def test_telegram_entry_tag_performance_handle(
    ...
    msg_mock.reset_mock()
    mocker.patch('coingro.rpc.rpc.RPC._rpc_enter_tag_performance',
...
def test_telegram_exit_reason_performance_handle(default_conf_usdt, update, ticker, fee,
    ...
    msg_mock.reset_mock()
    mocker.patch('coingro.rpc.rpc.RPC._rpc_exit_reason_performance',
                 side_effect=RPCException('Error'))
...
def test_telegram_mix_tag_performance_handle(default_conf_usdt, update, ticker, fee,
    ...
    msg_mock.reset_mock()
    mocker.patch('coingro.rpc.rpc.RPC._rpc_mix_tag_performance',
                 side_effect=RPCException('Error'))
...
def test_edge_enabled(edge_conf, update, mocker) -> None:
    mocker.patch('coingro.edge.Edge._cached_pairs', mocker.PropertyMock(
        return_value={
