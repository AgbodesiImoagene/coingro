...
from coingro.rpc.telegram import Telegram, authorized_only
...
def get_telegram_testobject(mocker, default_conf, mock=True, cgbot=None):
    msg_mock = MagicMock()
    if mock:
        mocker.patch.multiple(
            "coingro.rpc.telegram.Telegram", _init=MagicMock(), _send_msg=msg_mock
        )
    if not cgbot:
...
def test_telegram_init(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch("coingro.rpc.telegram.Updater", MagicMock(return_value=start_polling))
...
def test_cleanup(
    ...
    mocker.patch("coingro.rpc.telegram.Updater", updater_mock)
...
def test_telegram_status(default_conf, update, mocker) -> None:
    ...
    mocker.patch("coingro.rpc.telegram.Telegram._status_table", status_table)
...
def test_status_handle(default_conf, update, ticker, fee, mocker) -> None:
    ...
    status_table = MagicMock()
    mocker.patch.multiple(
        "coingro.rpc.telegram.Telegram",
        _status_table=status_table,
    )

    telegram, coingrobot, msg_mock = get_telegram_testobject(mocker, default_conf)
...
def test_daily_handle(default_conf_usdt, update, ticker, fee, mocker, time_machine) -> None:
    ...
    telegram, coingrobot, msg_mock = get_telegram_testobject(mocker, default_conf_usdt)
...
def test_daily_wrong_input(default_conf, update, ticker, mocker) -> None:
    mocker.patch.multiple("coingro.exchange.Exchange", fetch_ticker=ticker)
...
def test_profit_handle(
    ...
    mocker.patch("coingro.wallets.Wallets.get_starting_balance", return_value=1000)
    assert (
    ...
    mocker.patch("coingro.exchange.Exchange.fetch_ticker", ticker_sell_up)
...
def test_balance_handle_empty_response(default_conf, update, mocker) -> None:
    default_conf["dry_run"] = False
    mocker.patch("coingro.exchange.Exchange.get_balances", return_value={})

    telegram, coingrobot, msg_mock = get_telegram_testobject(mocker, default_conf)
    patch_get_signal(coingrobot)

    coingrobot.config["dry_run"] = False
    telegram._balance(update=update, context=MagicMock())
    result = msg_mock.call_args_list[0][0][0]
    assert msg_mock.call_count == 1
...
def test_balance_handle_too_large_response(default_conf, update, mocker) -> None:
    ...
    mocker.patch(
        "coingro.rpc.rpc.RPC._rpc_balance",
        return_value={
            "currencies": balances,
            "total": 100.0,
            "symbol": 100.0,
...
def test_telegram_forceexit_handle(
    ...
    mocker.patch("coingro.rpc.rpc.CryptoToFiatConverter._find_price", return_value=15000.0)
    msg_mock = mocker.patch("coingro.rpc.telegram.Telegram.send_msg", MagicMock())
    mocker.patch("coingro.rpc.telegram.Telegram._init", MagicMock())
    patch_exchange(mocker)
    ...
    coingrobot = CoingroBot(default_conf)
    rpc = RPC(coingrobot)
    telegram = Telegram(rpc, default_conf)
    patch_get_signal(coingrobot)
...
def test_telegram_force_exit_down_handle(
    default_conf, update, ticker, fee, ticker_sell_down, mocker
) -> None:
    mocker.patch("coingro.rpc.fiat_convert.CryptoToFiatConverter._find_price", return_value=15000.0)
    msg_mock = mocker.patch("coingro.rpc.telegram.Telegram.send_msg", MagicMock())
    mocker.patch("coingro.rpc.telegram.Telegram._init", MagicMock())
    patch_exchange(mocker)
    patch_whitelist(mocker, default_conf)
    ...
    mocker.patch.multiple("coingro.exchange.Exchange", fetch_ticker=ticker_sell_down)
...
def test_force_exit_no_pair(default_conf, update, ticker, fee, mocker) -> None:
    ...
    femock = mocker.patch("coingro.rpc.rpc.RPC._rpc_force_exit")
    telegram, coingrobot, msg_mock = get_telegram_testobject(mocker, default_conf)
...
def test_force_enter_handle(default_conf, update, mocker) -> None:
    ...
    telegram, coingrobot, _ = get_telegram_testobject(mocker, default_conf)
    patch_get_signal(coingrobot)
...
def test_telegram_entry_tag_performance_handle(
    ...
    msg_mock.reset_mock()
    mocker.patch(
        "coingro.rpc.rpc.RPC._rpc_enter_tag_performance", side_effect=RPCException("Error")
    )
...
def test_telegram_exit_reason_performance_handle(
    default_conf_usdt, update, ticker, fee, mocker
) -> None:
    mocker.patch.multiple(
        "coingro.exchange.Exchange",
    ...
    msg_mock.reset_mock()
    mocker.patch(
        "coingro.rpc.rpc.RPC._rpc_exit_reason_performance", side_effect=RPCException("Error")
    )
...
def test_telegram_mix_tag_performance_handle(
    default_conf_usdt, update, ticker, fee, mocker
) -> None:
    mocker.patch.multiple(
        "coingro.exchange.Exchange",
    ...
    assert "<code>TEST3 roi\t9.842 USDT (10.00%) (1)</code>" in msg_mock.call_args_list[0][0][0]
    ...
    msg_mock.reset_mock()
    mocker.patch("coingro.rpc.rpc.RPC._rpc_mix_tag_performance", side_effect=RPCException("Error"))
...
def test_blacklist_static(default_conf, update, mocker) -> None:
    ...
    assert coingrobot.pairlists.blacklist == ["DOGE/BTC", "HOT/BTC", "ETH/BTC"]
    ...
    assert coingrobot.pairlists.blacklist == ["DOGE/BTC", "HOT/BTC", "ETH/BTC", "XRP/.*"]
...
def test_telegram_logs(default_conf, update, mocker) -> None:
    ...
    assert msg_mock.call_count == 1
    assert "coingro\\.rpc\\.telegram" in msg_mock.call_args_list[0][0][0]
    ...
    mocker.patch("coingro.rpc.telegram.MAX_TELEGRAM_MESSAGE_LENGTH", 200)
    context = MagicMock()
    context.args = []
    telegram._logs(update=update, context=context)
...
def test_edge_enabled(edge_conf, update, mocker) -> None:
    ...
    mocker.patch("coingro.edge.Edge._cached_pairs", mocker.PropertyMock(return_value={}))
    telegram._edge(update=update, context=MagicMock())
    assert msg_mock.call_count == 1
...
def test_version_handle(default_conf, update, mocker) -> None:
    ...
    msg_mock.reset_mock()
    coingrobot.strategy.version = lambda: "1.1.1"
...
def test_show_config_handle(default_conf, update, mocker) -> None:
    ...
    msg_mock.reset_mock()
    coingrobot.config["trailing_stop"] = True
    telegram._show_config(update=update, context=MagicMock())
    assert msg_mock.call_count == 1
    assert "*Mode:* `{}`".format("Dry-run") in msg_mock.call_args_list[0][0][0]
    assert "*Exchange:* `binance`" in msg_mock.call_args_list[0][0][0]
...
def test_send_msg_buy_notification(
    ...
    coingrobot.config["telegram"]["notification_settings"] = {"buy": "off"}
    caplog.clear()
    msg_mock.reset_mock()
    telegram.send_msg(msg)
    ...
    coingrobot.config["telegram"]["notification_settings"] = {"buy": "silent"}
    caplog.clear()
    msg_mock.reset_mock()
...
def test_send_msg_sell_fill_notification(
...
def test__send_msg_keyboard(default_conf, mocker, caplog) -> None:
    ...
    coingrobot.config["telegram"]["enabled"] = True
    telegram = init_telegram(coingrobot)
    telegram._send_msg("test")
    ...
    coingrobot.config["telegram"]["enabled"] = True
    coingrobot.config["telegram"]["keyboard"] = invalid_keys_list
    err_msg = (
        re.escape(
    ...
    coingrobot.config["telegram"]["enabled"] = True
    coingrobot.config["telegram"]["keyboard"] = custom_keys_list
    telegram = init_telegram(coingrobot)
    telegram._send_msg("test")
    used_keyboard = bot.send_message.call_args[1]["reply_markup"]
