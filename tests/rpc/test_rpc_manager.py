...
def test__init__(mocker, default_conf) -> None:
    ...
    rpc_manager = RPCManager(get_patched_coingrobot(mocker, default_conf))
...
def test_cleanup_telegram_disabled(mocker, default_conf, caplog) -> None:
    caplog.set_level(logging.DEBUG)
    telegram_mock = mocker.patch("coingro.rpc.telegram.Telegram.cleanup", MagicMock())
    default_conf["telegram"]["enabled"] = False

    coingrobot = get_patched_coingrobot(mocker, default_conf)
    rpc_manager = RPCManager(coingrobot)
    rpc_manager.cleanup()
...
def test_send_msg_telegram_disabled(mocker, default_conf, caplog) -> None:
    telegram_mock = mocker.patch("coingro.rpc.telegram.Telegram.send_msg", MagicMock())
    default_conf["telegram"]["enabled"] = False
...
def test_send_msg_webhook_CustomMessagetype(mocker, default_conf, caplog) -> None:
    ...
    default_conf["telegram"]["enabled"] = False
    default_conf["webhook"] = {"enabled": True, "url": "https://DEADBEEF.com"}
    mocker.patch("coingro.rpc.webhook.Webhook.send_msg", MagicMock(side_effect=NotImplementedError))
    rpc_manager = RPCManager(get_patched_coingrobot(mocker, default_conf))
...
def test_startupmessages_telegram_enabled(mocker, default_conf, caplog) -> None:
    ...
    coingrobot = get_patched_coingrobot(mocker, default_conf)
    rpc_manager = RPCManager(coingrobot)
    rpc_manager.startup_messages(default_conf, coingrobot.pairlists, coingrobot.protections)
...
def test_init_apiserver_disabled(mocker, default_conf, caplog) -> None:
    caplog.set_level(logging.DEBUG)
    run_mock = MagicMock()
    mocker.patch("coingro.rpc.api_server.ApiServer.start_api", run_mock)
    default_conf["telegram"]["enabled"] = False
    rpc_manager = RPCManager(get_patched_coingrobot(mocker, default_conf))
