...
from coingro.enums import ExitType, RPCMessageType
from coingro.rpc import RPC
from coingro.rpc.discord import Discord
from coingro.rpc.webhook import Webhook
...
def test__init__(mocker, default_conf):
    ...
    webhook = Webhook(RPC(get_patched_coingrobot(mocker, default_conf)), default_conf)
...
def test_send_msg_webhook(default_conf, mocker):
    ...
    msg_mock = MagicMock()
    mocker.patch("coingro.rpc.webhook.Webhook._send_msg", msg_mock)
    webhook = Webhook(RPC(get_patched_coingrobot(mocker, default_conf)), default_conf)
    # Test buy
    msg_mock = MagicMock()
    mocker.patch("coingro.rpc.webhook.Webhook._send_msg", msg_mock)
    ...
    assert msg_mock.call_args[0][0]["value1"] == default_conf["webhook"]["webhookentry"][
        "value1"
    ].format(**msg)
    assert msg_mock.call_args[0][0]["value2"] == default_conf["webhook"]["webhookentry"][
        "value2"
    ].format(**msg)
    assert msg_mock.call_args[0][0]["value3"] == default_conf["webhook"]["webhookentry"][
        "value3"
    ].format(**msg)
    assert msg_mock.call_args[0][0]["value4"] == default_conf["webhook"]["webhookentry"][
        "value4"
    ].format(**msg)
    assert msg_mock.call_args[0][0]["value5"] == default_conf["webhook"]["webhookentry"][
        "value5"
    ].format(**msg)
    # Test short
    msg_mock.reset_mock()
...
def test__send_msg(default_conf, mocker, caplog):
    default_conf["webhook"] = get_webhook_dict()
    webhook = Webhook(RPC(get_patched_coingrobot(mocker, default_conf)), default_conf)
    msg = {"value1": "DEADBEEF", "value2": "ALIVEBEEF", "value3": "COINGRO"}
    post = MagicMock()
    mocker.patch("coingro.rpc.webhook.post", post)
...
def test_send_msg_discord(default_conf, mocker):
    ...
    msg_mock = MagicMock()
    mocker.patch("coingro.rpc.webhook.Webhook._send_msg", msg_mock)
    discord = Discord(RPC(get_patched_coingrobot(mocker, default_conf)), default_conf)
