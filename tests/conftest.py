...
def patched_configuration_load_config_file(mocker, config) -> None:
    mocker.patch(
        "coingro.configuration.load_config.load_config_file", lambda *args, **kwargs: config
    )
...
def patch_exchange(
    ...
    mocker.patch("coingro.exchange.Exchange._load_async_markets", MagicMock(return_value={}))
    mocker.patch("coingro.exchange.Exchange.validate_pairs", MagicMock())
    mocker.patch("coingro.exchange.Exchange.validate_timeframes", MagicMock())
    mocker.patch("coingro.exchange.Exchange.validate_ordertypes", MagicMock())
    mocker.patch("coingro.exchange.Exchange.validate_stakecurrency", MagicMock())
    mocker.patch("coingro.exchange.Exchange.validate_pricing")
    mocker.patch("coingro.exchange.Exchange.id", PropertyMock(return_value=id))
    mocker.patch("coingro.exchange.Exchange.name", PropertyMock(return_value=id.title()))
    mocker.patch("coingro.exchange.Exchange.precisionMode", PropertyMock(return_value=2))

    if mock_markets:
        if isinstance(mock_markets, bool):
            mock_markets = get_markets()
        mocker.patch("coingro.exchange.Exchange.markets", PropertyMock(return_value=mock_markets))

    if mock_supported_modes:
        mocker.patch(
            f"coingro.exchange.{id.capitalize()}._supported_trading_mode_margin_pairs",
            PropertyMock(
                return_value=[
    ...
    if api_mock:
        ...
        mocker.patch("coingro.exchange.Exchange._init_ccxt", MagicMock())
        mocker.patch(
            "coingro.exchange.Exchange.timeframes",
...
def patch_wallet(mocker, free=999.9) -> None:
    mocker.patch("coingro.wallets.Wallets.get_free", MagicMock(return_value=free))


def patch_whitelist(mocker, conf) -> None:
    mocker.patch(
        "coingro.coingrobot.CoingroBot._refresh_active_whitelist",
...
def patch_edge(mocker) -> None:
    ...
    mocker.patch("coingro.edge.Edge.calculate", MagicMock(return_value=True))


# Functions for recurrent object patching


def patch_coingrobot(mocker, config) -> None:
    """
    ...
    mocker.patch("coingro.coingrobot.RPCManager._init", MagicMock())
    mocker.patch("coingro.coingrobot.RPCManager.send_msg", MagicMock())
    patch_whitelist(mocker, config)


def get_patched_coingrobot(mocker, config) -> CoingroBot:
    """
    ...
    :return: CoingroBot
    """
    patch_coingrobot(mocker, config)
    config["datadir"] = Path(config["datadir"])
    return CoingroBot(config)
...
def patch_get_signal(
    ...
    coingro.strategy.get_entry_signal = patched_get_entry_signal
    ...
    # returns (enter, exit)
    coingro.strategy.get_exit_signal = patched_get_exit_signal

    coingro.exchange.refresh_latest_ohlcv = lambda p: None
...
def leverage_tiers():
    return {
        "1000SHIB/USDT": [
            {"min": 0, "max": 50000, "mmr": 0.01, "lev": 50, "maintAmt": 0.0},
            {"min": 50000, "max": 150000, "mmr": 0.025, "lev": 20, "maintAmt": 750.0},
            {"min": 150000, "max": 250000, "mmr": 0.05, "lev": 10, "maintAmt": 4500.0},
            {"min": 250000, "max": 500000, "mmr": 0.1, "lev": 5, "maintAmt": 17000.0},
            {"min": 500000, "max": 1000000, "mmr": 0.125, "lev": 4, "maintAmt": 29500.0},
            {"min": 1000000, "max": 2000000, "mmr": 0.25, "lev": 2, "maintAmt": 154500.0},
            {"min": 2000000, "max": 30000000, "mmr": 0.5, "lev": 1, "maintAmt": 654500.0},
        ],
        "1INCH/USDT": [
            {"min": 0, "max": 5000, "mmr": 0.012, "lev": 50, "maintAmt": 0.0},
            {"min": 5000, "max": 25000, "mmr": 0.025, "lev": 20, "maintAmt": 65.0},
            {"min": 25000, "max": 100000, "mmr": 0.05, "lev": 10, "maintAmt": 690.0},
            {"min": 100000, "max": 250000, "mmr": 0.1, "lev": 5, "maintAmt": 5690.0},
            {"min": 250000, "max": 1000000, "mmr": 0.125, "lev": 2, "maintAmt": 11940.0},
            {"min": 1000000, "max": 100000000, "mmr": 0.5, "lev": 1, "maintAmt": 386940.0},
        ],
        "AAVE/USDT": [
            {"min": 0, "max": 50000, "mmr": 0.01, "lev": 50, "maintAmt": 0.0},
            {"min": 50000, "max": 250000, "mmr": 0.02, "lev": 25, "maintAmt": 500.0},
            {"min": 250000, "max": 1000000, "mmr": 0.05, "lev": 10, "maintAmt": 8000.0},
            {"min": 1000000, "max": 2000000, "mmr": 0.1, "lev": 5, "maintAmt": 58000.0},
            {"min": 2000000, "max": 5000000, "mmr": 0.125, "lev": 4, "maintAmt": 108000.0},
            {"min": 5000000, "max": 10000000, "mmr": 0.1665, "lev": 3, "maintAmt": 315500.0},
            {"min": 10000000, "max": 20000000, "mmr": 0.25, "lev": 2, "maintAmt": 1150500.0},
            {"min": 20000000, "max": 50000000, "mmr": 0.5, "lev": 1, "maintAmt": 6150500.0},
        ],
        "ADA/BUSD": [
            {"min": 0, "max": 100000, "mmr": 0.025, "lev": 20, "maintAmt": 0.0},
            {"min": 100000, "max": 500000, "mmr": 0.05, "lev": 10, "maintAmt": 2500.0},
            {"min": 500000, "max": 1000000, "mmr": 0.1, "lev": 5, "maintAmt": 27500.0},
            {"min": 1000000, "max": 2000000, "mmr": 0.15, "lev": 3, "maintAmt": 77500.0},
            {"min": 2000000, "max": 5000000, "mmr": 0.25, "lev": 2, "maintAmt": 277500.0},
            {"min": 5000000, "max": 30000000, "mmr": 0.5, "lev": 1, "maintAmt": 1527500.0},
        ],
        "BNB/BUSD": [
            {
