...
def test_informative_decorator(mocker, default_conf_usdt, trading_mode):
    ...
    assert len(strategy._cg_informative) == 6  # Equal to number of decorators used
    informative_pairs = [
    ...
    mocker.patch(
        "coingro.data.dataprovider.DataProvider.historic_ohlcv", side_effect=test_historic_ohlcv
    )
