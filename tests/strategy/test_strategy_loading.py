    ...
    default_conf.update(
        {
            ...
            "strategy_path": str(Path(__file__).parents[2] / "coingro/templates"),
        }
    )
...
def test_load_strategy_base64(result, caplog, default_conf):
    filepath = Path(__file__).parents[2] / "coingro/templates/sample_strategy.py"
...
def test_load_strategy_invalid_directory(result, caplog, default_conf):
    ...
    with pytest.raises(OperationalException):
        StrategyResolver._load_strategy(
            CURRENT_TEST_STRATEGY, config=default_conf, extra_dir=extra_dir
        )
...
def test_strategy_override_trading_mode(caplog, default_conf):
    ...
    assert log_has("Override strategy 'trading_mode' with value in config file: margin.", caplog)
    assert strategy.margin_mode == "cross"
    assert log_has("Override strategy 'margin_mode' with value in config file: cross.", caplog)
