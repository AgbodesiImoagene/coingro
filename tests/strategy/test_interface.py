...
from coingro.enums import ExitCheckTuple, ExitType, SignalDirection
...
from coingro.strategy.parameters import (
    BaseParameter,
    BooleanParameter,
...
def test_ignore_expired_candle(default_conf):
    ...
    assert (
        not strategy.ignore_expired_candle(
            latest_date=latest_date, current_time=current_time, timeframe_seconds=300, enter=True
        )
        is True
    )
...
def test_advise_all_indicators_copy(mocker, default_conf, testdatadir) -> None:
    strategy = StrategyResolver.load_strategy(default_conf)
    aimock = mocker.patch("coingro.strategy.interface.IStrategy.advise_indicators")
    timerange = TimeRange.parse_timerange("1510694220-1510700340")
    data = load_data(testdatadir, "1m", ["UNITTEST/BTC"], timerange=timerange, fill_up_missing=True)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (0.05, 1, ExitType.NONE, False, True, 0.09, 1.04, ExitType.NONE, lambda **kwargs: -0.05),
        (
            0.05,
            0.95,
            ExitType.NONE,
        ...
        (0.05, 0.9, ExitType.NONE, False, True, 0.09, 0.9, ExitType.NONE, lambda **kwargs: None),
    ],
)
def test_stop_loss_reached(
    ...
    sl_flag = strategy.stop_loss_reached(
...
def test_auto_hyperopt_interface_loadparams(default_conf, mocker, caplog):
    ...
    mocker.patch("coingro.strategy.hyper.json_load", return_value=expected_result)
    PairLocks.timeframe = default_conf["timeframe"]
    strategy = StrategyResolver.load_strategy(default_conf)
    ...
    mocker.patch("coingro.strategy.hyper.json_load", MagicMock(side_effect=ValueError()))
