...
from coingro.edge import Edge, PairInfo
from coingro.enums import ExitType
from coingro.exceptions import OperationalException
from tests.conftest import get_patched_coingrobot, log_has
...
def test_edge_results(edge_conf, mocker, caplog, data) -> None:
    ...
    """
    coingro = get_patched_coingrobot(mocker, edge_conf)
    edge = Edge(edge_conf, coingro.exchange, coingro.strategy)
...
def test_edge_process_downloaded_data(mocker, edge_conf):
    coingro = get_patched_coingrobot(mocker, edge_conf)
    mocker.patch('coingro.exchange.Exchange.get_fee', MagicMock(return_value=0.001))
    mocker.patch('coingro.edge.edge_positioning.refresh_data', MagicMock())
    mocker.patch('coingro.edge.edge_positioning.load_data', mocked_load_data)
    edge = Edge(edge_conf, coingro.exchange, coingro.strategy)
...
def test_edge_process_no_data(mocker, edge_conf, caplog):
    ...
    mocker.patch('coingro.exchange.Exchange.get_fee', MagicMock(return_value=0.001))
    mocker.patch('coingro.edge.edge_positioning.refresh_data', MagicMock())
    mocker.patch('coingro.edge.edge_positioning.load_data', MagicMock(return_value={}))
    edge = Edge(edge_conf, coingro.exchange, coingro.strategy)
...
def test_edge_process_no_trades(mocker, edge_conf, caplog):
    coingro = get_patched_coingrobot(mocker, edge_conf)
    mocker.patch('coingro.exchange.Exchange.get_fee', return_value=0.001)
    mocker.patch('coingro.edge.edge_positioning.refresh_data', )
    mocker.patch('coingro.edge.edge_positioning.load_data', mocked_load_data)
    # Return empty
    mocker.patch('coingro.edge.Edge._find_trades_for_stoploss_range', return_value=[])
    edge = Edge(edge_conf, coingro.exchange, coingro.strategy)
...
def test_edge_process_no_pairs(mocker, edge_conf, caplog):
    ...
    mocker.patch('coingro.coingrobot.validate_config_consistency')

    coingro = get_patched_coingrobot(mocker, edge_conf)
    fee_mock = mocker.patch('coingro.exchange.Exchange.get_fee', return_value=0.001)
    mocker.patch('coingro.edge.edge_positioning.refresh_data')
    mocker.patch('coingro.edge.edge_positioning.load_data', mocked_load_data)
    # Return empty
    mocker.patch('coingro.edge.Edge._find_trades_for_stoploss_range', return_value=[])
...
def test_edge_init_error(mocker, edge_conf,):
    ...
    with pytest.raises(OperationalException,  match='Edge works only with unlimited stake amount'):
        get_patched_coingrobot(mocker, edge_conf)
...
def test_process_expectancy(mocker, edge_conf, fee, risk_reward_ratio, expectancy):
    ...
    coingro.exchange.get_fee = get_fee
    edge = Edge(edge_conf, coingro.exchange, coingro.strategy)
