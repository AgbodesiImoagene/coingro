...
from coingro.constants import AVAILABLE_PAIRLISTS
from coingro.enums import CandleType, RunMode
...
def test_log_cached(mocker, static_pl_conf, markets, tickers):
    ...
    coingro = get_patched_coingrobot(mocker, static_pl_conf)
    logmock = MagicMock()
    # Assign starting whitelist
    pl = coingro.pairlists._pairlist_handlers[0]
...
def test_load_pairlist_noexist(mocker, markets, default_conf):
    ...
    plm = PairListManager(coingro.exchange, default_conf)
    with pytest.raises(
        ...
        PairListResolver.load_pairlist(
            "NonexistingPairList", coingro.exchange, plm, default_conf, {}, 1
        )


def test_load_pairlist_verify_multi(mocker, markets_static, default_conf):
    coingro = get_patched_coingrobot(mocker, default_conf)
    mocker.patch("coingro.exchange.Exchange.markets", PropertyMock(return_value=markets_static))
    plm = PairListManager(coingro.exchange, default_conf)
    ...
    assert plm.verify_whitelist(["ETH/USDT", "XRP/USDT",], print) == [
        "ETH/USDT",
    ]
    assert plm.verify_whitelist(
        [
...
def test_refresh_market_pair_not_in_whitelist(mocker, markets, static_pl_conf):
    ...
    mocker.patch("coingro.exchange.Exchange.markets", PropertyMock(return_value=markets))
    coingro.pairlists.refresh_pairlist()
    # List ordered by BaseVolume
    whitelist = ["ETH/BTC", "TKN/BTC"]
    # Ensure all except those in whitelist are removed
    assert set(whitelist) == set(coingro.pairlists.whitelist)
    # Ensure config dict hasn't been changed
    assert (
        static_pl_conf["exchange"]["pair_whitelist"] == coingro.config["exchange"]["pair_whitelist"]
    )
...
def test_refresh_static_pairlist(mocker, markets, static_pl_conf):
    ...
    # Ensure all except those in whitelist are removed
    assert set(whitelist) == set(coingro.pairlists.whitelist)
    assert static_pl_conf["exchange"]["pair_blacklist"] == coingro.pairlists.blacklist
...
def test_refresh_static_pairlist_noexist(mocker, markets, static_pl_conf, pairs, expected, caplog):
    ...
    # Ensure all except those in whitelist are removed
    assert set(expected) == set(coingro.pairlists.whitelist)
    assert static_pl_conf["exchange"]["pair_blacklist"] == coingro.pairlists.blacklist
    if not expected:
...
def test_remove_logs_for_pairs_already_in_blacklist(mocker, markets, static_pl_conf, caplog):
    ...
    for _ in range(3):
        new_whitelist = coingro.pairlists.verify_blacklist(whitelist + ["BLK/BTC"], logger.warning)
...
def test_refresh_pairlist_dynamic(mocker, shitcoinmarkets, tickers, whitelist_conf):
    ...
    coingro = get_patched_coingrobot(mocker, whitelist_conf)
    # Remock markets with shitcoinmarkets since get_patched_coingrobot uses the markets fixture
    mocker.patch.multiple(
        "coingro.exchange.Exchange",
    ...
    coingro.pairlists.refresh_pairlist()
    assert whitelist == coingro.pairlists.whitelist
    ...
    with pytest.raises(
        ...
        PairListManager(coingro.exchange, whitelist_conf)
...
def test_refresh_pairlist_dynamic_2(mocker, shitcoinmarkets, tickers, whitelist_conf_2):
    ...
    mocker.patch.multiple(
        "coingro.plugins.pairlistmanager.PairListManager",
        _get_cached_tickers=MagicMock(return_value=tickers_dict),
    )
    coingro = get_patched_coingrobot(mocker, whitelist_conf_2)
    # Remock markets with shitcoinmarkets since get_patched_coingrobot uses the markets fixture
    mocker.patch.multiple(
        "coingro.exchange.Exchange",
        markets=PropertyMock(return_value=shitcoinmarkets),
...
def test_VolumePairList_refresh_empty(mocker, markets_empty, whitelist_conf):
    ...
    coingro = get_patched_coingrobot(mocker, whitelist_conf)
    mocker.patch("coingro.exchange.Exchange.markets", PropertyMock(return_value=markets_empty))
...
def test_VolumePairList_whitelist_gen(
    ...
    mocker.patch.multiple(
        "coingro.persistence.Trade", get_overall_performance=MagicMock(return_value=[])
    )
    ...
    if whitelist_result == "filter_at_the_beginning":
        ...
        coingro.pairlists.refresh_pairlist()
        whitelist = coingro.pairlists.whitelist
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "BTC",
            "ftx",
            ["HOT/BTC", "LTC/BTC", "ETH/BTC", "TKN/BTC", "XRP/BTC"],
        ),
    ],
)
def test_VolumePairList_range(
    ...
    mocker.patch("coingro.exchange.ftx.Ftx.market_is_tradable", return_value=True)
...
def test_gen_pair_whitelist_not_supported(mocker, default_conf, tickers) -> None:
    ...
    with pytest.raises(
        ...
        get_patched_coingrobot(mocker, default_conf)
...
def test_pairlist_class(mocker, whitelist_conf, markets, pairlist):
    ...
    assert coingro.pairlists.name_list == [pairlist]
    assert pairlist in str(coingro.pairlists.short_desc())
    assert isinstance(coingro.pairlists.whitelist, list)
    assert isinstance(coingro.pairlists.blacklist, list)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["ETH/BTC", "TKN/BTC", "XLTCUSDT"], "is not tradable with Coingro"),
    ],
)
def test__whitelist_for_active_markets(
    ...
    # Assign starting whitelist
    pairlist_handler = coingro.pairlists._pairlist_handlers[0]
...
def test__whitelist_for_active_markets_empty(mocker, whitelist_conf, pairlist, tickers):
    ...
    coingro = get_patched_coingrobot(mocker, whitelist_conf)
    mocker.patch.multiple(
        "coingro.exchange.Exchange", markets=PropertyMock(return_value=None), get_tickers=tickers
    )
    # Assign starting whitelist
    pairlist_handler = coingro.pairlists._pairlist_handlers[0]
...
def test_volumepairlist_invalid_sortvalue(mocker, whitelist_conf):
    ...
    with pytest.raises(OperationalException, match=r"key asdf not in .*"):
        get_patched_coingrobot(mocker, whitelist_conf)
...
def test_volumepairlist_caching(mocker, markets, whitelist_conf, tickers):
    ...
    coingro = get_patched_coingrobot(mocker, whitelist_conf)
    assert len(coingro.pairlists._pairlist_handlers[0]._pair_cache) == 0
    assert tickers.call_count == 0
    coingro.pairlists.refresh_pairlist()
    assert tickers.call_count == 1

    assert len(coingro.pairlists._pairlist_handlers[0]._pair_cache) == 1
    coingro.pairlists.refresh_pairlist()
    assert tickers.call_count == 1
...
def test_agefilter_caching(mocker, markets, whitelist_conf_agefilter, tickers, ohlcv_history):
    with time_machine.travel("2021-09-01 05:00:00 +00:00") as t:
        ...
        coingro = get_patched_coingrobot(mocker, whitelist_conf_agefilter)
        assert coingro.exchange.refresh_latest_ohlcv.call_count == 0
        coingro.pairlists.refresh_pairlist()
        assert len(coingro.pairlists.whitelist) == 3
        assert coingro.exchange.refresh_latest_ohlcv.call_count > 0

        coingro.pairlists.refresh_pairlist()
        assert len(coingro.pairlists.whitelist) == 3
        # Call to XRP/BTC cached
        assert coingro.exchange.refresh_latest_ohlcv.call_count == 2
        ...
        mocker.patch("coingro.exchange.Exchange.refresh_latest_ohlcv", return_value=ohlcv_data)
        coingro.pairlists.refresh_pairlist()
        assert len(coingro.pairlists.whitelist) == 3
        assert coingro.exchange.refresh_latest_ohlcv.call_count == 1
        ...
        mocker.patch("coingro.exchange.Exchange.refresh_latest_ohlcv", return_value=ohlcv_data)
        coingro.pairlists.refresh_pairlist()
        assert len(coingro.pairlists.whitelist) == 4
...
def test_rangestabilityfilter_caching(
    ...
    assert coingro.exchange.refresh_latest_ohlcv.call_count == 0
    coingro.pairlists.refresh_pairlist()
    assert len(coingro.pairlists.whitelist) == expected_length
    assert coingro.exchange.refresh_latest_ohlcv.call_count > 0

    previous_call_count = coingro.exchange.refresh_latest_ohlcv.call_count
    coingro.pairlists.refresh_pairlist()
    assert len(coingro.pairlists.whitelist) == expected_length
    # Should not have increased since first call.
    assert coingro.exchange.refresh_latest_ohlcv.call_count == previous_call_count
...
def test_spreadfilter_invalid_data(mocker, default_conf, markets, tickers, caplog):
    ...
    cgbot = get_patched_coingrobot(mocker, default_conf)
    cgbot.pairlists.refresh_pairlist()

    assert len(cgbot.pairlists.whitelist) == 5

    tickers.return_value["ETH/BTC"]["ask"] = 0.0
    del tickers.return_value["TKN/BTC"]
    del tickers.return_value["LTC/BTC"]
    mocker.patch.multiple("coingro.exchange.Exchange", get_tickers=tickers)

    cgbot.pairlists.refresh_pairlist()
    assert log_has_re(r"Removed .* invalid ticker data.*", caplog)

    assert len(cgbot.pairlists.whitelist) == 2
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "[{'PriceFilter': 'PriceFilter - Filtering pairs priced below 0.1% or below 0.00000010.'}]",  # noqa: E501
            None,
        ),
        (
            {"method": "PriceFilter", "low_price_ratio": 0.001, "max_price": 1.00010000},
            "[{'PriceFilter': 'PriceFilter - Filtering pairs priced below 0.1% or above 1.00010000.'}]",  # noqa: E501
            None,
        ),
        (
        ...
        (
            ...
            "[{'RangeStabilityFilter': 'RangeStabilityFilter - Filtering pairs with rate of change below "  # noqa: E501
...
def test_pricefilter_desc(
    ...
    if desc_expected is not None:
        coingro = get_patched_coingrobot(mocker, whitelist_conf)
        short_desc = str(coingro.pairlists.short_desc())
...
def test_performance_filter(
    ...
    coingro = get_patched_coingrobot(mocker, allowlist_conf)
    mocker.patch.multiple(
        "coingro.exchange.Exchange", get_tickers=tickers, markets=PropertyMock(return_value=markets)
    )
    mocker.patch.multiple(
        "coingro.exchange.Exchange",
