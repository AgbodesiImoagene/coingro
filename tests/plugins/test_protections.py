...
from tests.conftest import get_patched_coingrobot, log_has_re
...
def test_protectionmanager(mocker, default_conf):
    ...
    for handler in coingro.protections._protection_handlers:
...
def test_stoploss_guard(mocker, default_conf, fee, caplog, is_short):
    ...
    assert not coingro.protections.global_stop()
    ...
    assert coingro.protections.global_stop()
    ...
    coingro.protections.global_stop(end_time)
...
def test_stoploss_guard_perpair(mocker, default_conf, fee, caplog, only_per_pair, only_per_side):
    ...
    message = r"Trading stopped due to .*"
    pair = "XRP/BTC"
    assert not coingro.protections.stop_per_pair(pair)
    assert not coingro.protections.global_stop()
    assert not log_has_re(message, caplog)
    caplog.clear()
    ...
    # 3 Trades closed - but the 2nd has been closed too long ago.
    assert not coingro.protections.stop_per_pair(pair)
    assert coingro.protections.global_stop() != only_per_pair
    ...
    coingro.protections.stop_per_pair(pair)
    assert coingro.protections.global_stop() != only_per_pair
...
def test_CooldownPeriod(mocker, default_conf, fee, caplog):
    ...
    message = r"Trading stopped due to .*"
    assert not coingro.protections.global_stop()
    assert not coingro.protections.stop_per_pair("XRP/BTC")
    ...
    assert not coingro.protections.global_stop()
    assert coingro.protections.stop_per_pair("XRP/BTC")
    ...
    assert coingro.protections.stop_per_pair("ETH/BTC")
...
def test_LowProfitPairs(mocker, default_conf, fee, caplog, only_per_side):
    ...
    assert coingro.protections.stop_per_pair("XRP/BTC") != only_per_side
    ...
    assert coingro.protections.global_stop() != only_per_side
    assert coingro.protections.stop_per_pair("XRP/BTC") != only_per_side
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "[{'MaxDrawdown': 'MaxDrawdown - Max drawdown protection, stop trading if drawdown is > 0.0 "  # noqa: E501
            "within 60 minutes.'}]",
            None,
        ),
        (
...
def test_protection_manager_desc(
    ...
    short_desc = str(coingro.protections.short_desc())
