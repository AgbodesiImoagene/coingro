...
from coingro.constants import DUST_PER_COIN
from coingro.enums import RPCMessageType, SignalDirection, TradingMode
from coingro.exceptions import OperationalException
from coingro.misc import chunks, plural, round_coin_value
from coingro.persistence import Trade
from coingro.rpc import RPC, RPCException, RPCHandler
...
class Telegram(RPCHandler):
    ...
    def _prepare_entry_details(self, filled_orders: List, quote_currency: str, is_open: bool):
        ...
        for x, order in enumerate(filled_orders):
            if not order["cg_is_entry"] or order["is_open"] is True:
                continue
            ...
            if x == 0:
                ...
                if is_open:
                    lines.append(
                        "({})".format(
                            cur_entry_datetime.humanize(granularity=["day", "hour", "minute"])
                        )
                    )
                lines.append(
                    f"*Entry Amount:* {cur_entry_amount} ({order['cost']:.8f} {quote_currency})"
                ...
                lines.append(
                    f"*Average Entry Price:* {cur_entry_average} "
    ...
    def _status(self, update: Update, context: CallbackContext) -> None:
        ...
        try:
            ...
            for r in results:
                ...
                r["num_entries"] = len([o for o in r["orders"] if o["cg_is_entry"]])
    ...
    def _timeunit_stats(self, update: Update, context: CallbackContext, unit: str) -> None:
        ...
        vals = {
            ...
            "weeks": TimeunitMappings(
                "Monday", "Weekly", "weeks (starting from Monday)", "update_weekly", 8
            ),
    ...
    def _help(self, update: Update, context: CallbackContext) -> None:
        ...
        if self._rpc._coingro.trading_mode != TradingMode.SPOT:
            force_enter_text += (
    ...
    def _version(self, update: Update, context: CallbackContext) -> None:
        ...
        :return: None
        """
        strategy_version = self._rpc._coingro.strategy.version()
    ...
    def _show_config(self, update: Update, context: CallbackContext) -> None:
        ...
        :return: None
        """
        val = RPC._rpc_show_config(self._config, self._rpc._coingro.state)
