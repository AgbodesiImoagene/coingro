...
class RPC:
    ...
    def __init__(self, coingro) -> None:
        """
        Initializes all enabled rpc modules
        :param coingro: Instance of a coingro bot
        :return: None
        """
    ...
    def _rpc_trade_statistics(
        ...
        starting_balance = self._coingro.wallets.get_starting_balance()
    ...
    def _rpc_force_entry(self, pair: str, price: Optional[float], *,
        ...
        if not stake_amount:
            # gen stake amount
            stake_amount = self._coingro.wallets.get_trade_stake_amount(pair)
    ...
    def _rpc_delete(self, trade_id: int) -> Dict[str, Union[str, int]]:
        ...
        with self._coingro._exit_lock:
            ...
            if trade.open_order_id:
                try:
                    self._coingro.exchange.cancel_order(trade.open_order_id, trade.pair)
                    c_count += 1
                except (ExchangeError):
                    pass
    ...
    def _rpc_blacklist(self, add: List[str] = None) -> Dict:
        ...
        if add:
            for pair in add:
                if pair not in self._coingro.pairlists.blacklist:
                    try:
                        expand_pairlist([pair], self._coingro.exchange.get_markets().keys())
