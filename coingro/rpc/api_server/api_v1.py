...
def timeunit_profit(
    timeunit: TimeUnit, timescale: int = 1, rpc: RPC = Depends(get_rpc), config=Depends(get_config)
):
    timeframe = timeunit.value
    resp = rpc._rpc_timeunit_profit(
        timescale, config["stake_currency"], config.get("fiat_display_currency", ""), timeframe
    )
