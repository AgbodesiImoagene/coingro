...
from coingro.strategy.strategy_helper import merge_informative_pair
...
def informative(
    ...
    * {timeframe} - timeframe of informative dataframe.
    ...
    def decorator(fn: PopulateIndicators):
        informative_pairs = getattr(fn, "_cg_informative", [])
        informative_pairs.append(InformativeData(_asset, _timeframe, _fmt, _ffill, _candle_type))
        setattr(fn, "_cg_informative", informative_pairs)
        return fn
...
def _format_pair_name(config, pair: str) -> str:
    return pair.format(
        stake_currency=config["stake_currency"], stake=config["stake_currency"]
    ).upper()
