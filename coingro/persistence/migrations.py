...
def migrate_orders_table(engine, table_back_name: str, cols_order: List):
    ...
    cg_fee_base = get_column_def(cols_order, 'cg_fee_base', 'null')
    ...
    with engine.begin() as connection:
        connection.execute(text(f"""
            insert into orders (id, cg_trade_id, cg_order_side, cg_pair, cg_is_open, order_id,
            status, symbol, order_type, side, price, amount, filled, average, remaining, cost,
            stop_price, order_date, order_filled_date, order_update_date, cg_fee_base)
            select id, cg_trade_id, cg_order_side, cg_pair, cg_is_open, order_id,
            status, symbol, order_type, side, price, amount, filled, {average} average, remaining,
            cost, {stop_price} stop_price, order_date, order_filled_date,
            order_update_date, {cg_fee_base} cg_fee_base
    ...
    with engine.begin() as connection:
        connection.execute(
            text(
                """
                update orders
                set cg_is_open = false
                where cg_is_open = true and (cg_trade_id, order_id) not in (
                    select id, stoploss_order_id from trades where stoploss_order_id is not null
                ) and cg_order_side = 'stoploss'
        ...
        connection.execute(
            text(
                ...
                update orders
                set cg_is_open = false
                where cg_is_open = true
                and (cg_trade_id, order_id) not in (
