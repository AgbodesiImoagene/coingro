...
from coingro.data.metrics import (
    calculate_cagr,
    calculate_csum,
...
def store_backtest_stats(
    recordfilename: Path, stats: Dict[str, DataFrame], dtappendix: str
) -> None:
    """
...
def store_backtest_signal_candles(
    recordfilename: Path, candles: Dict[str, Dict], dtappendix: str
) -> Path:
    """
...
def text_table_add_metrics(strat_results: Dict) -> str:
    if len(strat_results["trades"]) > 0:
        ...
        drawdown_metrics.extend(
            [
                ...
                (
                    "Absolute Drawdown",
                    round_coin_value(
                        strat_results["max_drawdown_abs"], strat_results["stake_currency"]
                    ),
                ),
                (
                    "Drawdown high",
                    round_coin_value(
                        strat_results["max_drawdown_high"], strat_results["stake_currency"]
                    ),
                ),
                (
                    "Drawdown low",
                    round_coin_value(
                        strat_results["max_drawdown_low"], strat_results["stake_currency"]
                    ),
                ),
        ...
        # command stores these results and newer version of coingro must be able to handle old
        ...
        metrics = [
            ...
            (
                "Final balance",
                round_coin_value(strat_results["final_balance"], strat_results["stake_currency"]),
            ),
            (
                "Absolute profit ",
                round_coin_value(
            ...
            (
                "Total trade volume",
                round_coin_value(strat_results["total_volume"], strat_results["stake_currency"]),
            ),
            *short_metrics,
            ("", ""),  # Empty line to improve readability
            (
            ...
            (
                "Best day",
                round_coin_value(
                    strat_results["backtest_best_day_abs"], strat_results["stake_currency"]
                ),
            ),
            (
                "Worst day",
                round_coin_value(
                    strat_results["backtest_worst_day_abs"], strat_results["stake_currency"]
                ),
            ),
            (
                "Days win/draw/lose",
            ...
            *entry_adjustment_metrics,
            ...
            (
                "Min balance",
                round_coin_value(strat_results["csum_min"], strat_results["stake_currency"]),
            ),
            (
                "Max balance",
                round_coin_value(strat_results["csum_max"], strat_results["stake_currency"]),
            ),
            *drawdown_metrics,
        ...
        start_balance = round_coin_value(
            strat_results["starting_balance"], strat_results["stake_currency"]
        )
        stake_amount = (
            round_coin_value(strat_results["stake_amount"], strat_results["stake_currency"])
...
def show_backtest_result(
    ...
    if (
        results.get("results_per_enter_tag") is not None
...
def show_backtest_results(config: Dict, backtest_stats: Dict):
    ...
    if len(backtest_stats["strategy"]) > 1:
        ...
        print(
            f"{results['backtest_start']} -> {results['backtest_end']} |"
