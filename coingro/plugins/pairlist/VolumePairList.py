...
class VolumePairList(IPairList):
    def __init__(
        ...
        if not self._use_range and not (
            ...
            and self._exchange._cg_has["tickers_have_quoteVolume"]
        ):
            raise OperationalException(
    ...
    def filter_pairlist(self, pairlist: List[str], tickers: Dict) -> List[str]:
        ...
        if self._use_range:
            ...
            to_ms = (
                int(arrow.utcnow().floor("minute").shift(minutes=-self._tf_in_min).int_timestamp)
                * 1000
            )
            ...
            for i, p in enumerate(filtered_tickers):
                ...
                if pair_candles is not None and not pair_candles.empty:
                    if self._exchange._cg_has["ohlcv_volume_currency"] == "base":
                    ...
                    quoteVolume = (
                        pair_candles["quoteVolume"].rolling(self._lookback_period).sum().iloc[-1]
                    )
