...
class Encryption:
    ...
    def __init__(self, config: Dict[str, Any] = {}, key_str: str = "default_key") -> None:
        """
        ...
        self.__password = Encryption.shuffle(key_str.lower()).encode()
        ...
        if config.get("encryption", False):
            ...
            if isinstance(salt, bytes):
                ...
                raise OperationalException(
                    "Config with encrypted credentials " "must contain a valid salt. "
                )
        else:
    ...
    def get_encrypted_config(self) -> Dict[str, Any]:
        if not self.encrypted_config:
            self.encrypted_config = self._encrypt_config(
                deepcopy(self.plain_config), PROTECTED_CREDENTIALS
            )
            self.encrypted_config.update(
                {"encryption": True, "salt": base64.encodebytes(self.salt)}
            )
    ...
    def get_plain_config(self) -> Dict[str, Any]:
        if not self.plain_config:
            self.plain_config = self._decrypt_config(
                deepcopy(self.encrypted_config), PROTECTED_CREDENTIALS
            )
    ...
    def _encrypt_config(
        self, config: Dict[str, Any], credentials: Optional[Dict[str, Optional[dict]]]
    ) -> Dict[str, Any]:
        if credentials:
            for key in credentials:
                if credentials[key]:
                    ...
                    if not isinstance(val, str):
                        raise OperationalException(
                            f"{key} is not a string and " "cannot be encrypted. "
                        )
                    if val:
        ...
        if credentials:
            for key in credentials:
                if credentials[key]:
                    ...
                    if not isinstance(val, str):
                        raise OperationalException(
                            f"{key} is not a string and " "cannot be decrypted. "
                        )
                    if val:
