...
from coingro.configuration.deprecated_settings import process_deprecated_setting
from coingro.enums import RunMode, TradingMode
...
def _extend_validator(validator_class):
    """
    Extended validator for the Coingro configuration JSON Schema.
...
CoingroValidator = _extend_validator(Draft4Validator)
...
def _validate_whitelist(conf: Dict[str, Any]) -> None:
    ...
    for pl in conf.get("pairlists", [{"method": "StaticPairList"}]):
        if pl.get("method") == "StaticPairList" and not conf.get("exchange", {}).get(
            "pair_whitelist"
        ):
