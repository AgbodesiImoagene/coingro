...
class Configuration:
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        config = Encryption(config, __id__).get_plain_config()
    ...
    def _process_common_options(self, config: Dict[str, Any]) -> None:
        ...
        if (
            ...
            and self.args["db_url"] != constants.DEFAULT_DB_LIVE_URL
        ):
    ...
    def _process_optimize_options(self, config: Dict[str, Any]) -> None:
        ...
        self._args_to_config(
            config,
            argname="backtest_show_pair_list",
        ...
        self._args_to_config(
            config, argname="timeframe", logstring="Overriding timeframe with Command line argument"
        )
        ...
        self._args_to_config(
            config, argname="backtest_breakdown", logstring="Parameter --breakdown detected ..."
        )

        self._args_to_config(
            config, argname="backtest_cache", logstring="Parameter --cache={} detected ..."
        )

        self._args_to_config(
            config,
            argname="disableparamexport",
        ...
        self._args_to_config(
            config, argname="hyperopt_path", logstring="Using additional Hyperopt lookup path: {}"
        )

        self._args_to_config(
            config, argname="hyperoptexportfilename", logstring="Using hyperopt file: {}"
        )

        self._args_to_config(
            config,
            argname="epochs",
            logstring="Parameter --epochs detected ... " "Will run Hyperopt with for {} epochs ...",
        )

        self._args_to_config(
            config, argname="spaces", logstring="Parameter -s/--spaces detected: {}"
        )

        self._args_to_config(
            config, argname="print_all", logstring="Parameter --print-all detected ..."
        )
        ...
        self._args_to_config(
            config,
            argname="hyperopt_list_min_objective",
            logstring="Parameter --min-objective detected: {}",
        )

        self._args_to_config(
            config,
            argname="hyperopt_list_max_objective",
        ...
        self._args_to_config(
            config,
            argname="hyperopt_ignore_missing_space",
    ...
    def _process_plot_options(self, config: Dict[str, Any]) -> None:
        ...
        self._args_to_config(config, argname="trade_ids", logstring="Filtering on trade_ids: {}")

        self._args_to_config(config, argname="plot_limit", logstring="Limiting plot to: {}")

        self._args_to_config(
            config, argname="plot_auto_open", logstring="Parameter --auto-open detected."
        )

        self._args_to_config(config, argname="trade_source", logstring="Using trades from: {}")

        self._args_to_config(
            config, argname="prepend_data", logstring="Prepend detected. Allowing data prepending."
        )
        self._args_to_config(
            config, argname="erase", logstring="Erase detected. Deleting existing data."
        )

        self._args_to_config(
            config, argname="no_trades", logstring="Parameter --no-trades detected."
        )
        ...
        self._args_to_config(
            config, argname="include_inactive", logstring="Detected --include-inactive-pairs: {}"
        )

        self._args_to_config(
            config, argname="download_trades", logstring="Detected --dl-trades: {}"
        )

        self._args_to_config(
            config, argname="dataformat_ohlcv", logstring='Using "{}" to store OHLCV data.'
        )

        self._args_to_config(
            config, argname="dataformat_trades", logstring='Using "{}" to store trades data.'
        )
    ...
    def _process_analyze_options(self, config: Dict[str, Any]) -> None:
        self._args_to_config(
            config, argname="analysis_groups", logstring="Analysis reason groups: {}"
        )

        self._args_to_config(
            config, argname="enter_reason_list", logstring="Analysis enter tag list: {}"
        )

        self._args_to_config(
            config, argname="exit_reason_list", logstring="Analysis exit tag list: {}"
        )

        self._args_to_config(
            config, argname="indicator_list", logstring="Analysis indicator list: {}"
        )
    ...
    def db_url_from_config(config: Dict[str, Any]) -> str:
        if "db_url" in config:
            if not (
                config.get("dry_run", True) and config["db_url"] == constants.DEFAULT_DB_LIVE_URL
            ):
        ...
        return (
            constants.DEFAULT_DB_DRYRUN_URL
