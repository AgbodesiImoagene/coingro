...
class HDF5DataHandler(IDataHandler):
    ...
    def ohlcv_get_available_data(
        ...
        _tmp = [re.search(cls._OHLCV_REGEX, p.name) for p in datadir.glob("*.h5")]
        return [
            (
    ...
    def ohlcv_get_pairs(cls, datadir: Path, timeframe: str, candle_type: CandleType) -> List[str]:
        ...
        _tmp = [
            re.search(r"^(\S+)(?=\-" + timeframe + candle + ".h5)", p.name)
    ...
    def trades_get_pairs(cls, datadir: Path) -> List[str]:
        ...
        :return: List of Pairs
        """
        _tmp = [re.search(r"^(\S+)(?=\-trades.h5)", p.name) for p in datadir.glob("*trades.h5")]
