...
class Exchange:
    ...
    # This defines defaults, which can be selectively overridden by subclasses using _cg_has
    # or by specifying them in the configuration.
    _cg_has_default: Dict = {
    ...
    def __init__(self, config: Dict[str, Any], validate: bool = True) -> None:
        ...
        # Deep merge cg_has with default cg_has options
        self._cg_has = deep_merge_dicts(self._cg_has, deepcopy(self._cg_has_default))
        if self.trading_mode == TradingMode.FUTURES:
            self._cg_has = deep_merge_dicts(self._cg_has_futures, self._cg_has)
        if exchange_config.get("_cg_has_params"):
            self._cg_has = deep_merge_dicts(exchange_config.get("_cg_has_params"), self._cg_has)
            logger.info("Overriding exchange._cg_has with config params, result: %s", self._cg_has)

        # Assign this directly for easy access
        self._ohlcv_partial_candle = self._cg_has["ohlcv_partial_candle"]

        self._trades_pagination = self._cg_has["trades_pagination"]
        self._trades_pagination_arg = self._cg_has["trades_pagination_arg"]
        ...
        self.markets_refresh_interval: int = (
            exchange_config.get("markets_refresh_interval", 60) * 60
        )
    ...
    def _ccxt_config(self) -> Dict:
        ...
        if self.trading_mode == TradingMode.MARGIN:
            ...
            return {"options": {"defaultType": self._cg_has["ccxt_futures_name"]}}
        else:
            return {}
    ...
    def ohlcv_candle_limit(
        ...
        """
        return int(
            self._cg_has.get("ohlcv_candle_limit_per_timeframe", {}).get(
                timeframe, self._cg_has.get("ohlcv_candle_limit")
            )
        )
    ...
    def market_is_future(self, market: Dict[str, Any]) -> bool:
        return (
            market.get(self._cg_has["ccxt_futures_name"], False) is True
    ...
    def market_is_tradable(self, market: Dict[str, Any]) -> bool:
        """
        Check if the market symbol is tradable by Coingro.
    ...
    def _load_async_markets(self, reload: bool = False) -> None:
        try:
            if self._api_async:
                self.loop.run_until_complete(self._api_async.load_markets(reload=reload))
    ...
    def _load_markets(self) -> None:
        ...
        try:
            ...
            if self._cg_has["needs_trading_fees"]:
    ...
    def validate_timeframes(self, timeframe: Optional[str]) -> None:
        ...
        if timeframe and timeframe_to_minutes(timeframe) < 1:
            raise OperationalException("Timeframes < 1m are currently not supported by Coingro.")
    ...
    def validate_ordertypes(self, order_types: Dict) -> None:
        ...
        if order_types.get("stoploss_on_exchange") and not self._cg_has.get(
            "stoploss_on_exchange", False
        ):
    ...
    def validate_pricing(self, pricing: Dict) -> None:
        ...
        if not pricing.get("use_order_book", False) and (
            not self.exchange_has("fetchTicker") or not self._cg_has["tickers_have_price"]
        ):
    ...
    def validate_order_time_in_force(self, order_time_in_force: Dict) -> None:
        ...
        """
        if any(
            v not in self._cg_has["order_time_in_force"] for k, v in order_time_in_force.items()
        ):
            raise OperationalException(
    ...
    def validate_required_startup_candles(self, startup_candles: int, timeframe: str) -> int:
        ...
        if self._cg_has["ohlcv_has_history"]:
    ...
    def validate_trading_mode_and_margin_mode(
        ...
        """
        Checks if coingro can perform trades using the configured
        trading mode(Margin, Futures) and MarginMode(Cross, Isolated)
        Throws OperationalException:
            If the trading_mode/margin_mode type are not supported by coingro on this exchange
        """
        if trading_mode != TradingMode.SPOT and (
            ...
            raise OperationalException(
                f"Coingro does not support {mm_value} {trading_mode.value} on {self.name}"
            )
    ...
    def _get_stake_amount_limit(
        ...
        amount_reserve_percent = 1.0 + self._config.get(
        ...
        return (
            self._get_stake_amount_considering_leverage(
                max(stake_limits) * amount_reserve_percent, leverage or 1.0
            )
            if isMin
            else min(stake_limits)
        )
    ...
    def create_dry_run_order(
        ...
        if stop_loss:
            ...
            dry_order["cg_order_type"] = "stoploss"

        if dry_order["type"] == "market" and not dry_order.get("cg_order_type"):
    ...
    def check_dry_limit_order_filled(self, order: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if (
            ...
            and not order.get("cg_order_type")
        ):
            pair = order["symbol"]
    ...
    def fetch_dry_run_order(self, order_id) -> Dict[str, Any]:
        ...
        try:
            ...
            from coingro.persistence import Order
    ...
    def _get_stop_order_type(self, user_order_type) -> Tuple[str, str]:
        ...
        available_order_Types: Dict[str, str] = self._cg_has["stoploss_order_types"]
    ...
    def stoploss(
        ...
        requires `_cg_has['stoploss_order_types']` to be set as a dict mapping limit and market
        ...
        """
        if not self._cg_has["stoploss_on_exchange"]:
            raise OperationalException(f"stoploss is not implemented for {self.name}.")
    ...
    def fetch_l2_order_book(self, pair: str, limit: int = 100) -> dict:
        ...
        limit1 = self.get_next_limit_in_list(
            limit, self._cg_has["l2_limit_range"], self._cg_has["l2_limit_range_required"]
        )
        try:
    ...
    def get_rate(self, pair: str, refresh: bool, side: EntryExit, is_short: bool) -> float:
        """
        ...
        if conf_strategy.get("use_order_book", False):
            ...
            logger.debug(
                f"{name} price from orderbook {price_side_word}"
    ...
    def calculate_fee_rate(self, order: Dict) -> Optional[float]:
        ...
        if fee_curr in self.get_pair_base_currency(order["symbol"]):
            ...
            return (
                round(
                    self._contracts_to_amount(order["symbol"], order["fee"]["cost"])
                    / order["cost"],
                    8,
                )
                if order["cost"]
    ...
    def extract_cost_curr_rate(self, order: Dict) -> Tuple[float, str, Optional[float]]:
        ...
        """
        return (order["fee"]["cost"], order["fee"]["currency"], self.calculate_fee_rate(order))
    ...
    def get_historic_ohlcv_as_df(
        self, pair: str, timeframe: str, since_ms: int, candle_type: CandleType
    ) -> DataFrame:
        """
    ...
    async def _async_get_historic_ohlcv(
        ...
        input_coroutines = [
            ...
            for since in range(
                since_ms, until_ms or (arrow.utcnow().int_timestamp * 1000), one_call
            )
        ]
    ...
    def _build_coroutine(
        self, pair: str, timeframe: str, candle_type: CandleType, since_ms: Optional[int]
    ) -> Coroutine:

        if not since_ms and (
            self._cg_has["ohlcv_require_since"] or self.required_candle_call_count > 1
        ):
    ...
    def _now_is_time_to_refresh(self, pair: str, timeframe: str, candle_type: CandleType) -> bool:
        ...
        return not (
            (self._pairs_last_refresh_time.get((pair, timeframe, candle_type), 0) + interval_in_sec)
            >= arrow.utcnow().int_timestamp
        )
    ...
    async def _async_get_candle_history(
        ...
        try:
            ...
            params = deepcopy(self._cg_has.get("ohlcv_params", {}))
            candle_limit = self.ohlcv_candle_limit(
            ...
            if candle_type != CandleType.FUNDING_RATE:
                ...
                data = await self._api_async.fetch_funding_rate_history(
                    pair, since=since_ms, limit=candle_limit
                )
        ...
        while True:
            ...
            if t:
                ...
                if from_id == t[-1][1] or t[-1][0] > until:
                    logger.debug(
                        f"Stopping because from_id did not change. " f"Reached {t[-1][0]} > {until}"
                    )
    ...
    def get_liquidation_price(
        ...
        if self.trading_mode in TradingMode.SPOT:
            return None
        elif self.margin_mode == MarginMode.ISOLATED and self.trading_mode == TradingMode.FUTURES:
    ...
    def _fetch_and_calculate_funding_fees(
        ...
        timeframe = self._cg_has["mark_ohlcv_timeframe"]
        timeframe_ff = self._cg_has.get(
            "funding_fee_timeframe", self._cg_has["mark_ohlcv_timeframe"]
        )
        ...
        mark_comb: PairWithTimeframe = (
            pair,
            timeframe,
            CandleType.from_string(self._cg_has["mark_ohlcv_price"]),
        )
    ...
    def get_or_calculate_liquidation_price(
        ...
        if isolated_liq:
            ...
            isolated_liq = (
                isolated_liq - buffer_amount if is_short else isolated_liq + buffer_amount
            )
            return isolated_liq
        else:
            return None
    ...
    def dry_run_liquidation_price(
        ...
        if self.trading_mode == TradingMode.FUTURES and self.margin_mode == MarginMode.ISOLATED:
            ...
            if market["inverse"]:
                raise OperationalException("Coingro does not yet support inverse contracts")
...
def validate_exchanges(all_exchanges: bool) -> List[Tuple[str, bool, str]]:
    ...
    exchanges_valid = [(e, *validate_exchange(e)) for e in exchanges]
