...
from coingro.misc import pair_to_filename
...
def generate_profit_graph(
    ...
    fig["layout"].update(title="Coingro Profit plot")
    fig["layout"]["yaxis1"].update(title="Price")
...
def generate_plot_filename(pair: str, timeframe: str) -> str:
    ...
    file_name = "coingro-plot-" + pair_s + "-" + timeframe + ".html"
...
def plot_profit(config: Dict[str, Any]) -> None:
    ...
    store_plot_file(
        fig,
        filename="coingro-profit-plot.html",
        directory=config["user_data_dir"] / "plot",
