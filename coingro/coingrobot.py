"""
Coingro is the main module of this bot. It contains the class Coingro()
"""
import copy
import logging
import traceback
...
from coingro.rpc import RPCManager
...
class CoingroBot(LoggingMixin):
    """
    Coingro is the main class of the bot.
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        logger.info("Starting coingro %s", __version__)
        ...
        # initialization, even before Coingrobot has a chance to start its throttling,
        # so anything in the Coingrobot instance should be ready (initialized), including
    ...
    def notify_status(self, msg: str) -> None:
        ...
        """
        self.rpc.send_msg({"type": RPCMessageType.STATUS, "status": msg})
        self.message = msg
    ...
    def process(self) -> None:
        ...
        # Without this, coingro my try to recreate stoploss_on_exchange orders
        ...
        for order in orders:
            try:
                fo = self.exchange.fetch_order_or_stoploss_order(
                    order.order_id, order.cg_pair, order.cg_order_side == "stoploss"
                )
                ...
                self.update_trade_state(
                    ...
                    order.order_id,
                    fo,
                    stoploss_order=(order.cg_order_side == "stoploss"),
                )
    ...
    def handle_insufficient_funds(self, trade: Trade):
        ...
        for order in trade.orders:
            ...
            try:
                ...
                if order.cg_order_side == "stoploss":
                    if fo and fo["status"] == "open":
                        # Assume this as the open stoploss order
                        trade.stoploss_order_id = order.order_id
                elif order.cg_order_side == trade.exit_side:
                    if fo and fo["status"] == "open":
                        # Assume this as the open order
                        trade.open_order_id = order.order_id
                elif order.cg_order_side == trade.entry_side:
                    if fo and fo["status"] == "open":
                        trade.open_order_id = order.order_id
                if fo:
                    logger.info(f"Found {order} for trade {trade}.")
                    self.update_trade_state(
                        trade, order.order_id, fo, stoploss_order=order.cg_order_side == "stoploss"
                    )
    ...
    def check_and_call_adjust_trade_position(self, trade: Trade):
        ...
        min_stake_amount = self.exchange.get_min_pair_stake_amount(
            trade.pair, current_rate, self.strategy.stoploss
        )
        ...
        stake_amount = strategy_safe_wrapper(
            self.strategy.adjust_trade_position, default_retval=None
        )(
            trade=trade,
        ...
        if stake_amount is not None and stake_amount > 0.0:
            ...
            self.execute_entry(
                trade.pair, stake_amount, price=current_rate, trade=trade, is_short=trade.is_short
            )
        ...
        if stake_amount is not None and stake_amount < 0.0:
            ...
            logger.error(
                f"Unable to decrease trade position / sell partially"
    ...
    def execute_entry(
        ...
        *,
        ...
        if pos_adjust:
            logger.info(
                f"Position adjust: about to create a new order for {pair} with stake: "
    ...
    def _notify_enter(
        self, trade: Trade, order: Dict, order_type: Optional[str] = None, fill: bool = False
    ) -> None:
        """
    ...
    def create_stoploss_order(self, trade: Trade, stop_price: float) -> bool:
        ...
        try:
            ...
            self.execute_trade_exit(
                trade, trade.stop_loss, exit_check=ExitCheckTuple(exit_type=ExitType.EMERGENCY_EXIT)
            )
    ...
    def handle_stoploss_on_exchange(self, trade: Trade) -> bool:
        ...
        try:
            ...
            stoploss_order = (
                self.exchange.fetch_stoploss_order(trade.stoploss_order_id, trade.pair)
        ...
        if stoploss_order and stoploss_order["status"] in ("closed", "triggered"):
            ...
            self.update_trade_state(
                trade, trade.stoploss_order_id, stoploss_order, stoploss_order=True
            )
        ...
        if not stoploss_order:
            stoploss = (
                ...
                if self.edge
                else self.strategy.stoploss / trade.leverage
            )
            if trade.is_short:
        ...
        for trade in Trade.get_open_order_trades():
            ...
            if not_closed:
                if fully_cancelled or (
                    order_obj
                    and self.strategy.cg_check_timed_out(
    ...
    def replace_order(self, order: Dict, order_obj: Optional[Order], trade: Trade) -> None:
        ...
        if order_obj and latest_candle_close_date > order_obj.order_date_utc:
            ...
            adjusted_entry_price = strategy_safe_wrapper(
                self.strategy.adjust_entry_price, default_retval=order_obj.price
            )(
                trade=trade,
                order=order_obj,
                pair=trade.pair,
                ...
                current_order_rate=order_obj.price,
            ...
            if order_obj.price != adjusted_entry_price:
                ...
                self.handle_cancel_enter(trade, order, cancel_reason, replacing=replacing)
    ...
    def handle_cancel_enter(
        self, trade: Trade, order: Dict, reason: str, replacing: Optional[bool] = False
    ) -> bool:
        """
        ...
        if order["status"] not in constants.NON_OPEN_EXCHANGE_STATES:
            ...
            corder = self.exchange.cancel_order_with_result(
                trade.open_order_id, trade.pair, trade.amount
            )
        ...
        self._notify_enter_cancel(
            trade, order_type=self.strategy.order_types["entry"], reason=reason
        )
    ...
    def handle_cancel_exit(self, trade: Trade, order: Dict, reason: str) -> bool:
        ...
        if order["remaining"] == order["amount"] or order.get("filled") == 0.0:
            if not self.exchange.check_order_canceled_empty(order):
                try:
                    ...
                    co = self.exchange.cancel_order_with_result(
                        trade.open_order_id, trade.pair, trade.amount
                    )
                    trade.update_order(co)
                except InvalidOrderException:
        ...
        self.wallets.update()
        self._notify_exit_cancel(trade, order_type=self.strategy.order_types["exit"], reason=reason)
        return cancelled
    ...
    def apply_fee_conditional(
        ...
        if fee_abs != 0 and self.wallets.get_free(trade_base_currency) >= amount:
            ...
            logger.info(f"Applying fee on amount for {trade} " f"(from {amount} to {real_amount}).")
            return real_amount
        return amount
    ...
    def handle_order_fee(self, trade: Trade, order_obj: Order, order: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if not isclose(
                safe_value_fallback(order, "filled", "amount"),
                new_amount,
                abs_tol=constants.MATH_CLOSE_PREC,
            ):
                order_obj.cg_fee_base = trade.amount - new_amount
        except DependencyException as exception:
    ...
    def get_real_amount(self, trade: Trade, order: Dict, order_obj: Order) -> float:
        ...
        if self.exchange.order_has_fee(order):
            ...
            logger.info(
                f"Fee for Trade {trade} [{order_obj.cg_order_side}]: "
    ...
    def fee_detection_from_trades(
        ...
        if fee_abs != 0:
            return self.apply_fee_conditional(
                trade, trade_base_currency, amount=amount, fee_abs=fee_abs
            )
        else:
            return amount
